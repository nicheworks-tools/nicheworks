...
const filePath = path.join(process.cwd(), 'tools/construction-tools-atlas/data/tools.basic.json');
const TARGET = 5000;

const raw = fs.readFileSync(filePath, 'utf8');
const data = JSON.parse(raw);
if (!Array.isArray(data)) throw new Error('tools.basic.json must be a JSON array');

const existingIds = new Set(data.map((x) => x && x.id).filter(Boolean));

const domains = [
  ['大工', 'Carpentry', 'carpentry'], ['内装', 'Interior', 'interior'], ['外装', 'Exterior', 'exterior'],
  ['左官', 'Plastering', 'plastering'], ['塗装', 'Painting', 'painting'], ['電気', 'Electrical', 'electrical'],
  ['設備', 'MEP', 'mep'], ['配管', 'Plumbing', 'plumbing'], ['空調', 'HVAC', 'hvac'],
  ['防水', 'Waterproofing', 'waterproofing'], ['屋根', 'Roofing', 'roofing'], ['足場', 'Scaffolding', 'scaffolding'],
  ['型枠', 'Formwork', 'formwork'], ['鉄筋', 'Rebar', 'rebar'], ['コンクリート', 'Concrete', 'concrete'],
  ['土工', 'Earthwork', 'earthwork'], ['解体', 'Demolition', 'demolition'], ['測量', 'Surveying', 'surveying'],
  ['墨出し', 'Layout', 'layout'], ['安全', 'Safety', 'safety'], ['建具', 'Joinery', 'joinery'],
  ['床', 'Flooring', 'flooring'], ['天井', 'Ceiling', 'ceiling'], ['壁', 'Wall', 'wall'], ['基礎', 'Foundation', 'foundation']
];
...
const nouns = [
  ...
  ['カッター', 'utility knife'], ['脚立', 'stepladder'], ['はしご', 'ladder'], ['タッカー', 'stapler'], ['釘', 'nail'],
  ['ワッシャー', 'washer'], ['プレート', 'plate'], ['ブラケット', 'bracket'], ['レール', 'rail'], ['パネル', 'panel'],
  ['断熱材', 'insulation'], ['ボード', 'drywall board'], ['合板', 'plywood'], ['コンパネ', 'form plywood'], ['タイル', 'tile'],
  ['巾木', 'baseboard'], ['見切り', 'trim'], ['笠木', 'coping'], ['水切り', 'flashing'], ['コーナー材', 'corner bead'],
  ['スペーサー', 'spacer'], ['クランプ', 'clamp'], ['ジャッキ', 'jack'], ['チェーン', 'chain'], ['ワイヤー', 'wire'],
  ['バルブ', 'valve'], ['ポンプ', 'pump'], ['メーター', 'meter'], ['センサー', 'sensor'], ['カバー', 'cover'],
  ['キャップ', 'cap'], ['ホース', 'hose'], ['ノズル', 'nozzle'], ['フィルター', 'filter'], ['グリス', 'grease'],
  ['プライマー', 'primer'], ['錆止め', 'rust primer'], ['溶接棒', 'welding rod'], ['砥石', 'grinding wheel'], ['刃', 'blade'],
  ['替刃', 'replacement blade'], ['チップソー', 'tipped saw blade'], ['ビット', 'bit'], ['ソケット', 'socket'], ['レンチ', 'wrench']
];
...
const qualifiers = [
  ...
  ['確認用', 'inspection'], ['補修用', 'repair'], ['施工用', 'installation'], ['撤去用', 'removal'], ['仮止め用', 'temporary fixing'],
  ['精密', 'precision'], ['粗作業用', 'rough work'], ['耐熱', 'heat-resistant'], ['防錆', 'rust-resistant'], ['省施工', 'quick-install']
];
...
const actions = [
  ...
  ['接続する', 'connect', 'connection'], ['支持する', 'support', 'supporting'], ['調整する', 'adjust', 'adjustment'], ['清掃する', 'clean', 'cleaning']
];
...
function buildEntry(i) {
  ...
  const serialLabel = serial > 1 ? ` ${serial}` : '';
  const id = `term_${slug(d[2])}_${slug(q[1])}_${slug(n[1])}_${slug(a[2])}_${serial}`;
  const ja = `${d[0]}${q[0]}${n[0]}${serialLabel}`;
  const en = `${q[1]} ${d[1]} ${n[1]} ${a[1]} term${serialLabel}`.replace(/\s+/g, ' ').trim();
  ...
  return {
    ...
    meta: { generated: true, batch: 'direct-5000' }
  };
}
