...
return {lang:document.documentElement.lang||'',h1:[...document.querySelectorAll('h1')].filter(vis).length,overflow:Math.max(0,document.documentElement.scrollWidth-document.documentElement.clientWidth),unnamed,unlabel,pos,minWarn:warns.length?Math.min(...warns):null,en:[...document.querySelectorAll('[data-lang="en"]')].some(vis),ja:[...document.querySelectorAll('[data-lang="ja"]')].some(vis)};
`;
...
async function clickLang(c,lang){return c.js(String.raw`const l=arguments[0],v=e=>{const s=getComputedStyle(e),r=e.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>0&&r.height>0};const e=[...document.querySelectorAll('[data-lang="'+l+'"]')].find(v);if(!e)return false;e.click();return true;`,[lang]);}
