...
const ignoredNames = new Set([
  'SPEC.md', 'README.md', 'package-lock.json', 'package.json'
]);
const ignoredSegments = new Set([
  'tests', 'test', '__tests__', 'docs', 'fixtures', 'snapshots', 'node_modules'
]);
...
for (const slug of slugs) {
  ...
  for (const file of walk(toolDir)) {
    ...
    for (const match of text.matchAll(directStripe)) {
      add(slug, file, 'direct_stripe', match[0]);
    }

    for (const match of text.matchAll(offerSchema)) {
      add(slug, file, 'offer_schema', match[0]);
    }

    if (commercialContext.test(text)) {
      for (const match of text.matchAll(oldFixedPrice)) {
        add(slug, file, 'legacy_fixed_price', match[0]);
      }
    }
...
if (findings.length) {
  ...
  for (const slug of [...grouped.keys()].sort()) {
    console.error(`\n[${slug}]`);
    for (const finding of grouped.get(slug)) {
