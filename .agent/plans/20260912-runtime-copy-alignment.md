...
- [ ] Pattern Atlas EN/JA home pages: remove "production shell" / "functions later" wording because dataset, renderers, color editing, preview, and SVG/PNG/CSS export are already implemented.
- [ ] Pattern Atlas EN/JA usage pages: replace planned-MVP/localStorage language with current runtime behavior.
- [ ] Pattern Atlas SPEC: remove the now-resolved stale-copy mismatch note.
- [ ] Old Kanji Reference: stop labeling currently ungated CSV/JSON/Markdown/print actions as Pro-only.
- [ ] Old Kanji Reference: make the billing-unavailable panel describe only unavailable/future advanced Pro areas, not current free exports.
- [ ] Old Kanji Reference SPEC: remove the now-resolved UI/runtime export contradiction while preserving the current Free export contract.
- [ ] Keep Pattern Atlas render/export code and Old Kanji Reference export handlers unchanged.
