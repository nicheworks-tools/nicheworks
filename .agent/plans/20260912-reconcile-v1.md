...
- Implement CSV export with source row references and selected original identifying values.
...
- Treat same-side duplicate signatures as exclusive grouped outcomes. Reserve duplicate-member rows before ordinary matching so a source row cannot be duplicate plus another primary status.
...
- XLSX Summary must record filenames, mappings, and active matching settings so the exported workbook carries its reconciliation context.
...
- [x] Make duplicate signatures exclusive grouped results and exclude duplicate-member rows from all other primary outcomes.
...
- Duplicate rows previously could be emitted both as `duplicate` and as `candidate`/other statuses. Duplicate signatures are now grouped once, member rows are reserved before ordinary matching, and one source row belongs to at most one primary outcome.
- Dense same-value 1:1 graphs can create a quadratic number of acceptable edges even with indexed amount lookup. The engine now counts graph edges and throws `candidate_graph_too_large` before partial automatic matching once the configured budget is exceeded. The dense-graph regression now uses distinct amounts within a large tolerance so duplicate short-circuiting cannot mask this safety path.
...
- duplicate signatures produce one grouped duplicate result and member rows do not appear in another primary status;
- ambiguous candidate cases never auto-pick an arbitrary row;
...
- CSV export contains status, row references, normalized values, reason, and selected original identifying values from both sides;
