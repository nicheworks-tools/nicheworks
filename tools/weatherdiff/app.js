...
function calcDiff(a, b) {
  const diff = {};
  diff.tmax = Math.abs(a.tmax - b.tmax);
  diff.tmin = Math.abs(a.tmin - b.tmin);
  diff.pop = Math.abs(a.pop - b.pop);
  diff.wind = Math.abs(a.wind - b.wind);
...
async function run(lat, lon, name) {
  ...
  document.getElementById("locCoord").textContent = `lat ${lat.toFixed(2)} / lon ${lon.toFixed(2)}`;
  document.getElementById("locTz").textContent = ""; // API側で timezone自動反映済み
  ...
  // 国コード推定（超簡易：display_name から）
  const cc = name.toLowerCase().includes("japan") ? "jp" : "xx";
...
document.getElementById("geoBtn").onclick = () => {
  navigator.geolocation.getCurrentPosition(async (pos) => {
    ...
    const name = d.display_name || `${lat},${lon}`;
    run(lat, lon, name);
