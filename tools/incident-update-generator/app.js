(() => {
  ...
  const buildPublicReviewChecklist = (data) => {
    return [
      "- [ ] Facts match the incident commander / owner notes / 事実関係がインシデント責任者・オーナーの記録と一致している",
      "- [ ] Customer names, personal data, tokens, internal URLs, and vendor-confidential details are removed / 顧客名、個人情報、トークン、社内URL、ベンダー機密情報を削除している",
      "- [ ] Cause, recovery, and scope are not overstated beyond confirmed information / 原因、復旧状況、影響範囲を確認済み情報以上に断定していない",
      "- [ ] SLA, refund, compensation, and legal responsibility language is absent unless approved / SLA、返金、補償、法的責任の文言は承認がない限り含めていない",
      "- [ ] Security, legal, PR, and support owners reviewed the public text when relevant / 必要に応じてセキュリティ、法務、広報、サポート責任者が公開文を確認した",
      `- [ ] Next update time is present and realistic / 次回更新時刻が明記され現実的である: ${packValue(data.nextUpdate, "TBD")}`,
    ].join("\n");
  };

  const buildPostmortemOutline = (data) => {
    return [
      `# Postmortem outline / ポストモーテム概要 - ${packValue(data.service)}`,
      "",
      "## Summary / 概要",
      packValue(data.causeSummary, "Summarize the confirmed cause after investigation. / 調査後に確認済みの原因を要約してください。"),
      "",
      "## Impact / 影響",
      packValue(data.impact, "Describe affected users, components, and customer-visible symptoms. / 影響を受けたユーザー、コンポーネント、顧客に見える症状を記載してください。"),
      "",
      "## Timeline / タイムライン",
      `- Start / 開始: ${packValue(data.start, "TBD")}`,
      `- Recovery / 復旧: ${packValue(data.recoveryTime, "TBD")}`,
      `- Duration / 継続時間: ${packValue(data.duration, "TBD")}`,
      "",
      "## Detection and response / 検知と対応",
      packValue(data.mitigation, "Document detection signals, owners, mitigations, and validation steps. / 検知シグナル、担当者、緩和策、検証手順を記録してください。"),
      "",
      "## Root cause / 根本原因",
      packValue(data.causeSummary, "Add root cause once confirmed. / 確認後に根本原因を追記してください。"),
      "",
      "## Follow-up actions / 再発防止アクション",
      packValue(data.followUpPlan, "Add owners, due dates, and prevention actions. / 担当者、期限、再発防止策を追記してください。"),
    ].join("\n");
  };
  ...
  const buildMarkdownPack = () => {
    ...
    sections.push(
      ...
      "",
      "## Public review checklist / 公開前レビューチェックリスト",
      "",
      buildPublicReviewChecklist(data),
      "",
      "## Postmortem outline / ポストモーテム概要",
      "",
