...
function initI18n(elements) {
  ...
  const apply = (lang) => {
    ...
    if (elements.keywordInput) {
      elements.keywordInput.placeholder = TEXT[lang].placeholder;
    }

    if (elements.conditionEl) {
      elements.conditionEl.textContent = TEXT[lang].conditionAll;
    }
...
function updateConditions({ prefSelect, citySelect, keywordInput, conditionEl }) {
  ...
  conditionEl.textContent = TEXT[currentLang].condition({
    pref: prefSelect.value,
    city: citySelect.value,
