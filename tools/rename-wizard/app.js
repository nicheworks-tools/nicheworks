(function () {
  ...
  var t = {
    ...
    selected: isEn ? "Selected" : "選択",
    changedCount: isEn ? "changed" : "変更あり",
    unchangedCount: isEn ? "unchanged" : "変更なし",
    duplicateFixedCount: isEn ? "numbered" : "重複修正",
    duplicateCount: isEn ? "duplicates" : "重複候補",
    warningCount: isEn ? "warnings" : "警告",
  };
  ...
  function renderSummary(rows, hasNormalized) {
    ...
    if (!hasNormalized) {
      summaryBox.textContent = isEn
        ? "Selected: " + originalNames.length + " files"
        : "選択ファイル数：" + originalNames.length + "件";
      return;
    }
    ...
    summaryBox.textContent = isEn
      ? "Selected: " + originalNames.length + " / " + t.changedCount + ": " + changed + " / " + t.unchangedCount + ": " + unchanged + " / " + t.duplicateFixedCount + ": " + duplicateFixed + " / " + t.duplicateCount + ": " + duplicate + " / " + t.warningCount + ": " + warnings
  ...
  function renderPreview(rows) {
    ...
    originalNames.forEach(function (orig, index) {
      ...
      if (row) {
        ...
        if (row.normalized !== row.original) statusTd.appendChild(makeBadge(t.changed, "badge-changed"));
        else statusTd.appendChild(makeBadge(t.same, "badge-same"));
        ...
        if (row.warnings && row.warnings.length) {
          row.warnings.forEach(function (w) {
            warningTd.appendChild(makeBadge(warningLabels[w] || w, "badge-warning"));
          });
        } else {
  ...
  function applyNormalizeWidth(str) {
    try { return str.normalize("NFKC"); } catch (e) { return str; }
  }
  ...
  function buildRows() {
    ...
    if (!(optDeduplicate && optDeduplicate.checked)) {
      baseRows.forEach(function (row) { row.warnings = getNameWarnings(row.normalized); });
    }
  ...
  function rowsToTsv(rows) {
    return rows.map(function (row) { return row.original + "\t" + row.normalized; }).join("\n");
  }
  ...
  function rowsToCsv(rows) {
    var header = ["original", "normalized", "changed", "duplicate", "duplicate_fixed", "warnings"];
    var lines = [header.map(csvCell).join(",")];
    rows.forEach(function (row) {
      lines.push([
        row.original,
        row.normalized,
        row.original !== row.normalized ? "yes" : "no",
        row.duplicate ? "yes" : "no",
        row.duplicateFixed ? "yes" : "no",
        (row.warnings || []).join("|")
      ].map(csvCell).join(","));
    });
    return lines.join("\r\n");
  ...
  if (copyTsvButton) {
    copyTsvButton.addEventListener("click", function () {
      ...
      copyText(rowsToTsv(currentRows)).then(function (ok) {
        showCopyStatus(ok ? t.copyOk : t.copyFail, ok);
      });
    });
  }
