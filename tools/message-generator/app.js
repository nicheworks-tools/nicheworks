...
const UI = {
  ...
  en: {
    ...
    placeholder: "e.g. RSVP, thank you, delay apology",
    buttons: {
      generate: "Generate",
      copy: "Copy",
      save: "Save TXT",
...
const RELATIONSHIP_LINES = {
  ja: {
    friend: {
      high: "親しい間柄ではありますが、失礼のないよう、心を込めてお伝えします。",
      medium: "いつも気にかけてくれてありがとう。",
      casual: "いつもありがとう。"
    },
    family: {
      high: "家族として、日頃の支えに深く感謝しています。",
      medium: "いつも支えてくれてありがとう。",
      casual: "いつも助かってるよ。"
    },
    boss: {
      high: "日頃よりご指導を賜り、誠にありがとうございます。",
      medium: "いつもご指導いただきありがとうございます。",
      casual: "いつもありがとうございます。"
    },
    client: {
      high: "平素より格別のご高配を賜り、心より御礼申し上げます。",
      medium: "いつもお世話になっております。",
      casual: "いつもありがとうございます。"
    }
  },
  en: {
    friend: {
      high: "Although we know each other well, I wanted to write with care and respect.",
      medium: "Thank you, as always, for being there.",
      casual: "Thanks as always."
    },
    family: {
      high: "I am grateful for your continued support as family.",
      medium: "Thank you for always supporting me.",
      casual: "Thanks for always being there."
    },
    boss: {
      high: "Thank you very much for your continued guidance and support.",
      medium: "Thank you for your continued guidance.",
      casual: "Thank you as always."
    },
    client: {
      high: "Thank you very much for your continued trust and support.",
      medium: "Thank you for your continued support.",
      casual: "Thank you as always."
    }
  }
};
...
function getOutputLang(culture) {
  return culture === "japan" ? "ja" : "en";
}
...
function setLanguage(lang, shouldNotify = false) {
  ...
  const keywordInput = $("#keywords");
  if (keywordInput) keywordInput.placeholder = content.placeholder;
  ...
  if (shouldNotify && currentMessage) {
    showToast(content.languageChanged);
  }
}
...
function isContextValid(context) {
  return Boolean(context.purpose && context.culture && context.formality);
}
...
function keywordSentence(lang, keywords) {
  ...
  if (lang === "ja") {
    return `関連する内容：${items.join("・")}。`;
  }
  return `Related details: ${items.join(", ")}.`;
}
...
function generateMessage(context) {
  ...
  const bodyPool = TEMPLATES[outputLang][context.purpose][context.formality];
  const body = bodyPool[0];
  const lines = [];

  if (context.relationship && RELATIONSHIP_LINES[outputLang][context.relationship]) {
    lines.push(RELATIONSHIP_LINES[outputLang][context.relationship][context.formality]);
  }
  ...
  if (context.purpose === "apology") {
    lines.push(APOLOGY_REVIEW_LINES[outputLang]);
  }

  const fullText = [opening, lines.join("\n"), closing].filter(Boolean).join("\n\n");
  return { fullText, outputLang };
}
...
function saveTextFile(text, outputLang) {
  ...
  const filename = `message-generator-${outputLang}-${date}.txt`;
...
function bindEvents() {
  ...
  $("#copy-btn").addEventListener("click", async () => {
    if (!currentMessage) {
      showToast(UI[currentLang].noMessage);
      return;
    }
    try {
