(() => {
  ...
  function selections() {
    return { platform: $("platform").value, sourceType: $("sourceType").value, customDomain: $("customDomain").value === "yes", outputFolder: $("outputFolder").value };
  }
  ...
  function build() {
    ...
    return {
      ...
      diag,
      pack,
      free: [`# ${platform} Deploy Guide`, "", `## ${hChecklist}`, checklistText, "", `## ${hErrors}`, errorsText].join("\n"),
  ...
  function updatePro() {
    if ($("proState")) {
      $("proState").textContent = pro ? "Pro Active" : "Pro Inactive";
      $("proState").className = `pro-status ${pro ? "active" : "inactive"}`;
    }
    if ($("proLocked")) $("proLocked").style.display = pro ? "none" : "";
    if ($("proArea")) $("proArea").style.display = pro ? "" : "none";
  }
  ...
  function render() {
    ...
    if (pro) {
      ...
      $("proPack").value = data.pack;
    } else {
      $("diagnosis").value = msg("Pro解除後に表示されます。", "Visible after Pro unlock.");
      $("proPack").value = msg("Pro解除後に表示されます。", "Visible after Pro unlock.");
    }
  }
  ...
  function patchProUi() {
    ...
    if (input) {
      ...
      input.placeholder = "NW-PDG-XXXX-XXXX-XX";
    }
    const actions = document.querySelector("#proLocked .pro-actions");
    if (actions) {
      actions.innerHTML = "";
      ...
      buy.target = "_blank";
      buy.rel = "noopener";
      buy.textContent = "Buy Pro - $2.99";
      const unlock = document.createElement("a");
      unlock.className = "btn";
      unlock.href = "/pro/unlock/";
      unlock.textContent = "Pro unlock page";
      actions.append(buy, unlock);
    }
  }
  ...
  function init() {
    ...
    if ($("diagnosis")) $("diagnosis").value = msg("Pro解除後、生成すると症状別診断ツリーが表示されます。", "After Pro unlock, generate to show the diagnosis tree.");
    if ($("proPack")) $("proPack").value = msg("Pro解除後、生成すると引き継ぎパックが表示されます。", "After Pro unlock, generate to show the handoff pack.");

    $("generate").addEventListener("click", render);
    $("copyAll").addEventListener("click", async () => {
      if (!generated) return toast(msg("先に生成してください。", "Generate a checklist first."));
      toast(await copy(freeText) ? msg("コピーしました。", "Copied.") : msg("コピーに失敗しました。", "Copy failed."));
    });
    $("activatePro").addEventListener("click", () => setPro(true));
    $("clearPro").addEventListener("click", () => setPro(false));
    $("copyPro").addEventListener("click", async () => {
      if (!pro) return toast(msg("Pro解除後に使えます。", "Available after Pro unlock."));
      if (!generated) return toast(msg("先に生成してください。", "Generate a checklist first."));
      toast(await copy(proText) ? msg("Pro出力をコピーしました。", "Copied Pro output.") : msg("コピーに失敗しました。", "Copy failed."));
    });
    $("downloadMd").addEventListener("click", () => {
      if (!pro) return toast(msg("Markdown出力はPro解除後に使えます。", "Markdown export is available after Pro unlock."));
      if (!generated) return toast(msg("先に生成してください。", "Generate a checklist first."));
