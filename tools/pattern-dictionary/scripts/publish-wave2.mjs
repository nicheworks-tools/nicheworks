...
const stageProd=readJson('data/wave2-production-content.json');
const stageSource=readJson('data/wave2-source-verification.json');
const stageSearch=readJson('data/wave2-search-dictionary.json');
const stageCompare=readJson('data/wave2-compare-guides.json');
const stageCases=readJson('tests/wave2-search-cases.json');
const stageRefs=readJson('data/wave2-reference-images.json');
const stageReview=readJson('data/wave2-reference-image-review.json');
...
if(stageProd.patterns?.length!==20||stageSource.patterns?.length!==20||stageRefs.images?.length!==20||stageReview.records?.length!==20)throw new Error('Wave 2 staging artifacts must contain exactly 20 records each');
const waveIds=stageSource.patterns.slice().sort((a,b)=>a.ordinal-b.ordinal).map(x=>x.pattern_id);
if(basePatterns.length===40&&waveIds.every(id=>basePatterns.some(x=>x.id===id))){console.log('Wave 2 already published; no transform needed.');process.exit(0)}
if(basePatterns.length!==20)throw new Error(`expected pre-publication runtime count 20, got ${basePatterns.length}`);
...
function deriveUses(p){
  ...
  const out=[];
  if(/shirt|jacket|suit|trouser|skirt|dress|tie|scarf|coat|hat|shorts|fashion|シャツ|ジャケット|スーツ|パンツ|スカート|ワンピース|ネクタイ|スカーフ|コート|帽子/.test(t))out.push('fashion');
  if(/textile|fabric|cloth|patchwork|生地|布|織物|パッチワーク/.test(t))out.push('textile');
  if(/interior|furnish|curtain|bedding|upholstery|インテリア|カーテン|寝具|家具/.test(t))out.push('interior');
  return out.length?out:['fashion','textile'];
}
function finalProduction(id){
  ...
  const q=s.verification_state==='qualified'?{ja:p.definition.ja,en:s.qualification}:undefined;
  return {
    pattern_id:id,
...
prod.updated=today;
prod.policy={...prod.policy,record_count:40,review_state:'verified',image_state:'verified-publication',qualified_terms:[...new Set([...prod.patterns.filter(x=>x.verification_state==='qualified').map(x=>x.pattern_id),...finalWaveProd.filter(x=>x.verification_state==='qualified').map(x=>x.pattern_id)])].sort()};
...
source.policy={...source.policy,purpose:'Evidence ledger for the published canonical 40-pattern production set.',qualification_rule:stageSource.policy?.qualification_rule||'Use qualified when a term does not resolve to one uniquely fixed motif.'};
...
review.scope='canonical-40';
review.wave2_review_history=stageReview.review_history;
review.records=uniq([...review.records,...stageReview.records],x=>x.pattern_id);
writeJson('data/reference-image-review.json',review);

const mergedSearch={...search,ja:{...(search.ja||{}),...(stageSearch.ja||{})},en:{...(search.en||{}),...(stageSearch.en||{})}};
writeJson('data/search-dictionary.json',mergedSearch);

compare.updated=today;
compare.policy={...compare.policy,purpose:'Explain decisive visual differences for high-confusion comparisons across the published canonical 40.'};
compare.guides=uniq([...compare.guides.filter(x=>x.id!=='argyle--diamond-family'),...stageCompare.guides],x=>x.id);
...
expansion.base={...expansion.base,published_count:40,rule:'The published canonical 40 remain in data/patterns.json. This file now tracks only remaining ordinals 41-100.'};
expansion.policy={...expansion.policy,expansion_count:60,waves:[3,4,5],publication_rule:'Remaining planned entries do not enter runtime patterns.json until their production wave completes source verification, bilingual content, reference image review, search regression, SEO, commerce mapping, and QA.'};
expansion.entries=expansion.entries.filter(x=>x.ordinal>=41);
writeJson('data/canonical-100-expansion.json',expansion);

const appPath=path.join(root,'app.js');
let app=fs.readFileSync(appPath,'utf8');
const allIds=[...basePatterns.map(x=>x.id),...waveIds];
const set=`const REFERENCE_IDS=new Set([${allIds.map(x=>`'${x}'`).join(',')}]);`;
if(!/const REFERENCE_IDS=new Set\(\[.*?\]\);/s.test(app))throw new Error('app.js REFERENCE_IDS contract not found');
app=app.replace(/const REFERENCE_IDS=new Set\(\[.*?\]\);/s,set);
fs.writeFileSync(appPath,app);

const stylePath=path.join(root,'style.css');
let css=fs.readFileSync(stylePath,'utf8');
if(!css.includes('.pd-mini-stripe'))css=css.replace('.pd-mini-geometric{','.pd-mini-stripe{background:repeating-linear-gradient(90deg,#111 0 4px,#fff 4px 9px)}.pd-mini-dot{background:radial-gradient(circle,#111 0 3px,transparent 4px) 0 0/10px 10px}.pd-mini-geometric{');
...
for(const rel of ['index.html','en/index.html']){
  ...
  if(rel==='index.html'){
    html=html.replace('現在の20件は','現在の40件は');
    if(!html.includes('data-family="stripe"'))html=html.replace('<button class="pd-filter" data-family="geometric"','<button class="pd-filter" data-family="stripe" aria-pressed="false"><span class="pd-mini pd-mini-stripe" aria-hidden="true"></span>ストライプ</button><button class="pd-filter" data-family="dot" aria-pressed="false"><span class="pd-mini pd-mini-dot" aria-hidden="true"></span>ドット</button><button class="pd-filter" data-family="geometric"');
  }else{
    html=html.replace('The current 20 entries','The current 40 entries');
    if(!html.includes('data-family="stripe"'))html=html.replace('<button class="pd-filter" data-family="geometric"','<button class="pd-filter" data-family="stripe" aria-pressed="false"><span class="pd-mini pd-mini-stripe" aria-hidden="true"></span>Stripes</button><button class="pd-filter" data-family="dot" aria-pressed="false"><span class="pd-mini pd-mini-dot" aria-hidden="true"></span>Dots</button><button class="pd-filter" data-family="geometric"');
  }
...
function detailHtml(id,lang){
  ...
  const ja=s.verified_names.ja,en=s.verified_names.en;
  const isJa=lang==='ja';
  ...
  const footer=isJa?'この項目は出典レビューとReference Image構造レビューを完了し、公開QAを通過しています。':'This entry has completed source review and Reference Image structural review and passed publication QA.';
...
readme=readme.replace('The **published runtime set remains exactly 20 patterns**. No planned 21-100 entry is exposed merely because it is present in the expansion manifest or a research ledger.','The **published runtime set is now exactly 40 patterns** after Wave 2 (entries 21-40) completed source verification, bilingual content, Reference Image review, runtime search/compare integration, Amazon commerce mapping, static-route SEO, and desktop/mobile QA.');
readme=readme.replace(/- \*\*Wave 2 \/ entries 21-40 source verification is complete as research-only:\*\* 13 verified and 7 qualified\. None of these 20 entries is public yet\./,'- **Wave 2 / entries 21-40 is published:** 13 verified and 7 qualified source terms, with all 20 Reference Images reviewed and runtime-wired.');
readme=readme.replace(/With the canonical-100 freeze and Wave 2 source verification complete, the next unit is \*\*Wave 2 production for entries 21-40\*\*:[\s\S]*?until that whole wave closes\./,'Wave 2 is published. The next production unit is **Wave 3 / entries 41-60**, which remains non-public until its own source, content, image, search, comparison, commerce, SEO, and browser-QA gates close.');
fs.writeFileSync(readmePath,readme);

console.log(`Published Wave 2 transform: runtime ${basePatterns.length} -> ${basePatterns.length+waveIds.length}; offers +${newOffers.length}; compare guides ${compare.guides.length}; new detail pages 40.`);
