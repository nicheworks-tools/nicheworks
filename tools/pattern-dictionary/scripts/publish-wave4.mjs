...
const stageProd=read('data/wave4-production-content.json');
const stageSource=read('data/wave4-source-verification.json');
const stageSearch=read('data/wave4-search-dictionary.json');
const stageCompare=read('data/wave4-compare-guides.json');
const stageCases=read('tests/wave4-search-cases.json');
const stageRefs=read('data/wave4-reference-images.json');
const stageReview=read('data/wave4-reference-image-review.json');
...
if(stageProd.patterns?.length!==20||stageSource.patterns?.length!==20||stageRefs.images?.length!==20||stageReview.records?.length!==20)throw new Error('Wave 4 staging artifacts must contain exactly 20 records each');
if(stageReview.status!=='structural-review-complete'||stageReview.records.some(x=>x.status!=='accepted'))throw new Error('Wave 4 structural review must be accepted before publication');
const waveRows=[...stageSource.patterns].sort((a,b)=>a.ordinal-b.ordinal);
const waveIds=waveRows.map(x=>x.pattern_id);
if(JSON.stringify(waveRows.map(x=>x.ordinal))!==JSON.stringify(Array.from({length:20},(_,i)=>61+i)))throw new Error('Wave 4 ordinals must be 61..80');
if(base.length===80&&waveIds.every(id=>base.some(x=>x.id===id))){console.log('Wave 4 already published; no transform needed.');process.exit(0)}
if(base.length!==60)throw new Error(`expected pre-publication runtime count 60, got ${base.length}`);
...
prod.policy={...prod.policy,record_count:80,review_state:'verified',image_state:'verified-publication',qualified_terms:[...new Set([...prod.patterns.filter(x=>x.verification_state==='qualified').map(x=>x.pattern_id),...waveProd.filter(x=>x.verification_state==='qualified').map(x=>x.pattern_id)])].sort()};
prod.patterns=uniq([...prod.patterns,...waveProd],x=>x.pattern_id);write('data/production-content.json',prod);

source.updated=today;source.policy={...source.policy,purpose:'Evidence ledger for the published canonical 80-pattern production set.',qualification_rule:stageSource.policy?.qualification_rule||'Use qualified when a term does not resolve to one uniquely fixed motif.'};
...
review.updated=today;review.scope='canonical-80';review.wave4_review_history=stageReview.review_history;
...
compare.updated=today;compare.policy={...compare.policy,purpose:'Explain decisive visual differences for high-confusion comparisons across the published canonical 80.'};compare.guides=uniq([...compare.guides,...stageCompare.guides],x=>x.id);write('data/compare-guides.json',compare);
...
expansion.version=today;expansion.base={...expansion.base,published_count:80,rule:'The published canonical 80 remain in data/patterns.json. This file now tracks only remaining ordinals 81-100.'};
expansion.policy={...expansion.policy,expansion_count:20,waves:[5],publication_rule:'Remaining planned entries do not enter runtime patterns.json until their production wave completes source verification, bilingual content, reference image review, search regression, SEO, commerce mapping, and QA.'};
expansion.entries=expansion.entries.filter(x=>x.ordinal>=81);write('data/canonical-100-expansion.json',expansion);

const appPath=path.join(root,'app.js');let app=fs.readFileSync(appPath,'utf8');const ids80=[...base.map(x=>x.id),...waveIds];
if(!/const REFERENCE_IDS=new Set\(\[.*?\]\);/s.test(app))throw new Error('app.js REFERENCE_IDS contract not found');
app=app.replace(/const REFERENCE_IDS=new Set\(\[.*?\]\);/s,`const REFERENCE_IDS=new Set([${ids80.map(x=>`'${x}'`).join(',')}]);`);fs.writeFileSync(appPath,app);
...
for(const rel of ['index.html','en/index.html']){
  ...
  if(rel==='index.html'){
    html=html.replace(/現在の60件/g,'現在の80件');
    ...
    html=html.replace(/The current 60 entries/g,'The current 80 entries');
...
function detailHtml(id,lang){
  ...
  const ads='<script async src="https://pagead2.googlesyndication.com/pagead/js?client=ca-pub-9879006623791275" crossorigin="anonymous"></script>',beacon='<script defer src="https://static.cloudflareinsights.com/beacon.min.js" data-cf-beacon=\'{"token":"aeec938336694c99bc864cdf859b5e37"}\'></script>';
...
readme=readme.replace(/published 60-pattern visual dictionary/g,'published 80-pattern visual dictionary');
readme=readme.replace(/remaining 40 canonical entries/g,'remaining 20 canonical entries');
readme=readme.replace(/published runtime set is exactly 60 patterns/g,'published runtime set is exactly 80 patterns');
readme=readme.replace(/all 60 published terms/g,'all 80 published terms');
readme=readme.replace(/remaining ordinals \*\*61-100\*\*/g,'remaining ordinals **81-100**');
if(!readme.includes('Wave 4 / entries 61-80 is published'))readme+='\n- **Wave 4 / entries 61-80 is published:** 20 source-verified records, 20 reviewed deterministic Reference Images, 23 additional comparison guides, 66 staged search regressions, and 60 fixed Amazon commerce intents.\n- **Wave 5 / entries 81-100** is the final remaining production unit.\n';
fs.writeFileSync(readmePath,readme);

console.log(`Published Wave 4 transform: runtime 60 -> 80; offers +${offers.length}; compare guides ${compare.guides.length}; new detail pages 40; remaining canonical expansion ${expansion.entries.length}.`);
