...
if (!['candidates-complete','structural-review-complete'].includes(manifest.status)) throw new Error(`unexpected manifest status ${manifest.status}`);
...
for (const image of manifest.images) {
  ...
  if (image.review_state === 'reviewed' && reviewById[image.pattern_id]?.decision !== 'pass') throw new Error(`${image.pattern_id}: reviewed state requires a passing structural review record`);
}
...
console.log(`OK: 20/20 Reference Images are 1536x1536 PNG, image_ready-or-later, structurally reviewed where marked reviewed, and runtime-wired.`);
