...
if(published.length!==20)throw new Error(`Wave 2 staging must not publish runtime records; got ${published.length}`);
if(prod.phase!=='wave2-production-content'||prod.publication_state!=='staged-not-public')throw new Error('Wave 2 production pack must remain staged-not-public');
...
if(frozen.length!==20||ledger.length!==20||rows.length!==20)throw new Error(`expected 20/20/20 frozen/source/production rows, got ${frozen.length}/${ledger.length}/${rows.length}`);
const allIds=new Set([...published.map(x=>x.id),...canonical.entries.map(x=>x.id)]);
for(let i=0;i<20;i++){
  const f=frozen[i],s=ledger[i],r=rows[i];
  if(r.ordinal!==f.ordinal||r.pattern_id!==f.id||s.pattern_id!==f.id)throw new Error(`ordinal/id mismatch at ${f.ordinal}`);
  ...
  for(const rel of [...(r.relationships?.similar||[]),...(r.relationships?.often_confused_with||[])])if(!allIds.has(rel))throw new Error(`${r.pattern_id}: unresolved relationship ${rel}`);
...
if(compare.publication_state!=='staged-not-public'||compare.guides.length<10)throw new Error('Wave 2 compare plan must contain at least 10 staged guides');
for(const g of compare.guides){
  if(!allIds.has(g.left)||!allIds.has(g.right))throw new Error(`${g.id}: compare endpoint outside canonical 100`);
...
for(const lang of ['ja','en'])for(const [term,v] of Object.entries(dict[lang]||{})){
  ...
  for(const id of v.ids||[])if(!allIds.has(id))throw new Error(`${term}: unresolved staged search id ${id}`);
}
...
const stagedPatterns=rows.map(r=>{
  const s=ledger.find(x=>x.pattern_id===r.pattern_id);
  return {...r,names:s.verified_names,aliases:s.verified_aliases,term_scope:s.term_scope,colors:{primary:s.color_guidance.primary},culture:r.families.includes('japanese')?['Japanese']:[]};
});
...
console.log(`OK: Wave 2 production pack covers 20/20, ${compare.guides.length} compare guides, ${cases.length} staged search regressions, and maintained 2-4 Amazon intents per pattern; runtime remains 20.`);
