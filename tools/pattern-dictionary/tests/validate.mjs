...
for(const x of p){
  ...
  if(x.review_state!=='researched') errors.push(`${x.id}: expected researched review state, got ${x.review_state}`);
  for(const [lang,rel] of [['ja',path.join('patterns',x.id,'index.html')],['en',path.join('en','patterns',x.id,'index.html')]]){
    ...
    if(!html.includes('noindex,follow')) errors.push(`${rel}: researched detail must stay noindex until image verification`);
  }
}
...
console.log('OK: 20 researched canonical records, 40 static noindex detail pages, and required production-phase files.');
