...
if(current.length!==60)throw new Error(`canonical published base must be exactly 60 after Wave 3, got ${current.length}`);
if(expansion.schema!=='pattern-dictionary-canonical-100-v1')throw new Error('unexpected canonical-100 schema');
if(expansion.base?.published_count!==60||expansion.policy?.expansion_count!==40||expansion.policy?.target_total!==100)throw new Error('canonical-100 counts are inconsistent after Wave 3 publication');
...
if(entries.length!==40)throw new Error(`expected 40 remaining expansion entries, got ${entries.length}`);
const existingIds=current.map(x=>x.id),plannedIds=entries.map(x=>x.id),allIds=[...existingIds,...plannedIds];
if(new Set(existingIds).size!==60)throw new Error('published canonical IDs must be unique');
if(new Set(plannedIds).size!==40)throw new Error('remaining planned canonical IDs must be unique');
if(new Set(allIds).size!==100)throw new Error('canonical 100 contains duplicate IDs');

for(let i=0;i<entries.length;i++){
  const e=entries[i],ordinal=i+61,wave=Math.floor((ordinal-1)/20)+1;
...
for(const wave of [4,5])if(entries.filter(x=>x.wave===wave).length!==20)throw new Error(`wave ${wave}: expected 20 remaining entries`);
...
for(const id of ['dalmatian-spots','nami-chidori','tomoe','baroque-scroll'])if(!plannedIds.includes(id))throw new Error(`missing relationship-driven remaining entry: ${id}`);
for(const id of ['hexagon','hishi','ivy'])if(!existingIds.includes(id))throw new Error(`Wave 3 relationship-driven entry must now be published: ${id}`);
...
for(const id of ['chinoiserie','flame-stitch','moire','kanoko','same-komon','shibori','batik','bandhani','bogolan','adire','sashiko','block-print']){
...
console.log(`OK: canonical 100 is now 60 published + 40 planned; waves 4-5 remain 20 each; ${risky.length} remaining broad terms stay explicitly qualified.`);
