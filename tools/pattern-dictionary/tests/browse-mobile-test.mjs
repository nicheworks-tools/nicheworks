...
const expectedFamilies=['check','stripe','dot','geometric','japanese','floral','ornamental','animal'];

if(patterns.length!==60)throw new Error(`expected 60 patterns, got ${patterns.length}`);
if(new Set(patterns.map(x=>x.id)).size!==60)throw new Error('pattern ids must be unique');
...
for(const file of indexes){
  ...
  if(html.indexOf('data-family="check"')>html.indexOf('id="pattern-grid"'))throw new Error(`${path.relative(root,file)} filters must appear before the 60-image browse grid`);
...
if(!css.includes('.pd-mini-stripe')||!css.includes('.pd-mini-dot')||!css.includes('.pd-mini-floral'))throw new Error('expanded visual filter cues missing');
...
console.log('OK: browse families, 120 detail wrappers, stale messaging, and mobile comparison/filter contracts passed.');
