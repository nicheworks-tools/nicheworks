...
const expectedFamilies=['check','stripe','dot','geometric','japanese','ornamental','animal'];

if(patterns.length!==40)throw new Error(`expected 40 patterns, got ${patterns.length}`);
if(new Set(patterns.map(x=>x.id)).size!==40)throw new Error('pattern ids must be unique');
...
for(const file of indexes){
  ...
  if(html.indexOf('data-family="check"')>html.indexOf('id="pattern-grid"'))throw new Error(`${path.relative(root,file)} filters must appear before the 40-image browse grid`);
...
if(!css.includes('.pd-mini-stripe')||!css.includes('.pd-mini-dot'))throw new Error('new stripe/dot filter cues missing');
...
console.log('OK: browse families, 80 detail wrappers, stale messaging, and mobile comparison/filter contracts passed.');
