...
(() => {
  ...
  const defaultState = { target: "laundry", drying: "outdoor", temp: 22, humidity: 58, wind: 3.2 };
  const labels = {
    target: {
  ...
  function text(pair) { return pair[currentLang] || pair.ja || pair.en || ""; }
  ...
  function adviceFor(score) {
    ...
    return {
      laundry: {
        do: { ja: bad || veryHumid ? "外干しだけに頼らず、除湿＋送風に切り替える。" : good ? "朝〜昼に外干しし、間隔を広めに取る。" : "外干し後に室内送風で仕上げる。", en: bad || veryHumid ? "Do not rely on outdoor drying alone; use dehumidification and airflow." : good ? "Dry outdoors from morning to noon with wide spacing." : "Finish with indoor airflow after outdoor drying." },
        avoid: { ja: humid || cold ? "密集干し、夕方までの放置、厚手との混在。" : "重ね干し、風の弱い場所、取り込み忘れ。", en: humid || cold ? "Crowding, leaving until evening, and mixing with thick items." : "Overlapping, windless spots, and forgetting retrieval." }
      },
      bedding: {
        do: { ja: score < 45 ? "今日は見送り、布団乾燥機や除湿仕上げを優先する。" : "短時間で裏返しながら干し、取り込み後に湿りを確認する。", en: score < 45 ? "Skip outdoor drying today and use a bedding dryer or dehumidifying finish." : "Dry briefly, flip during drying, and check for dampness after retrieval." },
  ...
  function bindEvents() {
    $$('.nw-lang-switch button').forEach((button) => {
      button.addEventListener("click", () => { applyLang(button.dataset.lang); renderResult(); });
    });
    $$('[data-target]').forEach((button) => {
      button.addEventListener("click", () => { state.target = ["laundry", "thick", "bedding"].includes(button.dataset.target) ? button.dataset.target : "laundry"; renderResult(); });
    });
    $$('[data-dry]').forEach((button) => {
      button.addEventListener("click", () => { state.drying = button.dataset.dry === "indoor" ? "indoor" : "outdoor"; renderResult(); });
    });
    const tempEl = $("#temp");
    const humidityEl = $("#humidity");
    const windEl = $("#wind");
    tempEl?.addEventListener("input", () => { state.temp = clamp(parseInt(tempEl.value, 10), -10, 45); renderResult(); });
    ...
    $$('.preset-btn').forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (preset === "winter-indoor") { state.temp = 5; state.humidity = 55; state.wind = 1; state.drying = "indoor"; }
        if (preset === "humid-day") { state.temp = 18; state.humidity = 88; state.wind = 2; state.drying = "indoor"; }
        if (preset === "dry-breezy") { state.temp = 26; state.humidity = 45; state.wind = 5.5; state.drying = "outdoor"; }
        renderResult();
      });
    });
