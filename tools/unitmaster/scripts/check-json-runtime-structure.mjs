...
if (index) {
  ...
  if (adapterIndex !== -1 && jsonRuntimeIndex !== -1 && adapterIndex > jsonRuntimeIndex) {
    fail('runtime-units-adapter.js must be loaded before app-json-runtime.js.');
  }
  if (legacyIndex !== -1) {
    fail('index.html must not directly load ./app.js after switching to JSON runtime. app.js is only a fallback loaded by app-json-runtime.js.');
  }
}
...
if (jsonRuntime) {
  if (!jsonRuntime.includes('./data/units.json')) {
    fail('app-json-runtime.js must reference ./data/units.json.');
  }
  if (!jsonRuntime.includes('./app.js')) {
    fail('app-json-runtime.js must keep ./app.js as the legacy fallback path.');
  }
  if (!jsonRuntime.includes('UnitMasterRuntimeAdapter')) {
    fail('app-json-runtime.js must use UnitMasterRuntimeAdapter.');
  }
}
