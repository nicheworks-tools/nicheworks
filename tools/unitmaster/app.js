...
function text(key) { return (labels[currentLang] && labels[currentLang][key]) || labels.ja[key] || key; }
function unitLabel(cat, unit) { if (cat === "temp") return unit.toUpperCase(); return (unitLabels[currentLang][cat] && unitLabels[currentLang][cat][unit]) || unit; }
...
function updateStaticText() {
  ...
  Array.from(categorySelect.options).forEach((option) => { option.textContent = categoryLabel(option.value); });
  tabs.forEach((tab) => { tab.textContent = categoryLabel(tab.dataset.cat); });
}
...
function applyCategory(cat, preserveUnits = false, saveAutoHistory = true) {
  ...
  keys.forEach((unit) => { addOption(fromSel, unit, unitLabel(safeCat, unit)); addOption(toSel, unit, unitLabel(safeCat, unit)); });
  if (preserveUnits) {
    if (keys.includes(prevFrom)) fromSel.value = prevFrom;
    if (keys.includes(prevTo)) toSel.value = prevTo;
  } else if (keys.length > 1) { toSel.value = keys[1]; }
  calculate(false);
...
function setResult(message, state) { resultBox.textContent = message; resultBox.classList.toggle("is-warning", state === "warning"); resultBox.classList.toggle("is-error", state === "error"); }
...
function generateBulkList(validation) {
  if (!validation.ok) { bulkBox.replaceChildren(); return; }
  const cat = categorySelect.value;
  ...
  if (cat === "temp") {
    units.temp.forEach((unit) => { if (unit !== fromSel.value) fragment.appendChild(createTextElement("div", "bulk-line", `= ${formatNumber(convertTemperature(validation.value, fromSel.value, unit))} ${unitLabel(cat, unit)}`)); });
  } else {
    const dict = units[cat];
    const base = validation.value * dict[fromSel.value];
    Object.keys(dict).forEach((unit) => { if (unit !== fromSel.value) fragment.appendChild(createTextElement("div", "bulk-line", `= ${formatNumber(base / dict[unit])} ${unitLabel(cat, unit)}`)); });
  }
...
function loadHistory() {
  const list = readHistory();
  const fragment = document.createDocumentFragment();
  if (list.length === 0) { fragment.appendChild(createTextElement("p", "history-empty", text("history_empty"))); historyBox.replaceChildren(fragment); return; }
...
function calculate(shouldSave) {
  ...
  if (!validation.ok) { latestValidResult = null; setResult(validation.message, validation.state); generateBulkList(validation); loadHistory(); return; }
  ...
  if (shouldSave) saveHistory(result); else loadHistory();
}
...
[inputValue, fromSel, toSel].forEach((el) => {
  el.addEventListener(el === inputValue ? "input" : "change", () => {
    if (autoCalc.checked) { calculate(false); if (latestValidResult) scheduleHistorySave(); }
  });
});
autoCalc.addEventListener("change", () => {
  calcBtn.classList.toggle("hidden", autoCalc.checked);
  if (autoCalc.checked) { calculate(false); if (latestValidResult) scheduleHistorySave(); } else { clearTimeout(historyTimer); }
});
calcBtn.addEventListener("click", () => { clearTimeout(historyTimer); calculate(true); });
