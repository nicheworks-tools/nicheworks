...
- The current public `storm` flow calls the precipitation metadata endpoint only for reachability status after validation.
- Therefore the visible `storm` replay is synthetic/placeholder preview data, not observed precipitation.

### `compare`

- `compare` builds deterministic A/B/Diff grids through `buildCompareData` and related rendering helpers.
- The UI labels visible A/B/Diff maps and CSV grid values as synthetic preview content in Japanese and English.
- The current public `compare` flow calls the precipitation metadata endpoint separately for Period A and Period B only to display metadata reachability.
- Therefore visible `compare` maps/diffs are synthetic/placeholder preview data, not observed precipitation.
...
- `card` remains disconnected from the real metadata endpoint. `compare` is covered by EMS-RD-05 and remains metadata-only for A/B reachability, with synthetic A/B/Diff previews.
...
## EMS-RD-05B update — Compare defaults and metadata panel copy

Date: 2026-05-12

- Compare default/example setup uses the metadata-safe Tokyo request: bbox `139.5,35.4,140.0,35.9`, Period A `2025-08-01` to `2025-08-03`, Period B `2025-08-04` to `2025-08-06`, preset `low`.
