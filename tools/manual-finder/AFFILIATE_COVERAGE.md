...
All 14 actionable Nikon camera records are closed. DJI Waves 1–2 add six reviewed detail rows, leaving DJI 90, OM SYSTEM 37, GoPro 31, and Insta360 7 missing accessory-detail rows.
...
5. **Camera batteries / chargers** — Nikon is complete at 14/14 detail. DJI has six reviewed rows across Osmo Action Wave 1 and Air Wave 2; the remaining camera backlog is measured separately and must continue in bounded reviewed waves.
...
The Wave 1 contract is exact-model only. The older `Osmo Action` and `DJI Action 2` records remain fail-closed for accessory detail until separately reviewed. This reduces DJI missing accessory detail from 96 to 92 without generalizing compatibility across the Osmo Action family.
...
The Wave 2 contract is exact-model only. `DJI Air 2S`, `Mavic Air 2`, and `Mavic Air` remain fail-closed for accessory detail until separately reviewed. DJI now reconciles at:

`DJI camera 96 = detail 6 + reviewed exclusion 0 + missing 90`

The catalog-wide camera audit now reports 185 actionable basic records, 20 detail mappings, 0 reviewed exclusions, and 165 missing accessory-detail rows. The remaining missing counts are DJI 90, OM SYSTEM 37, GoPro 31, and Insta360 7. Exact missing model arrays remain machine-readable in `tests/camera-accessory-coverage.test.mjs` output and are summarized in `CAMERA_ACCESSORY_COVERAGE.md`.
...
- `affiliate-dji-camera-accessories-wave2.js` adds the two reviewed DJI Air rows and exposes the merged 20-row camera detail ledger.
- `affiliate-camera-detail-exclusions.js` is the reviewed camera-detail exclusion ledger and is currently empty.
- `affiliate-runtime.js` renders the generic/body search plus zero or more verified consumable or camera-accessory searches. It loads Nikon Wave 1, Nikon Wave 2, DJI Wave 1, and DJI Wave 2 before initial affiliate rendering so the final config is complete before cards are mounted.
...
- DJI camera missing accessory detail = 90
...
Nikon camera coverage is closed. The measured camera backlog continues with DJI 90 actionable records, followed by OM SYSTEM 37, GoPro 31, and Insta360 7, using bounded reviewed product-family waves and exact manufacturer evidence. Appliance replacement-part rules remain a separate future phase.
