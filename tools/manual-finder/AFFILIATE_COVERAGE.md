...
| 41 | `Osmo Nano`, `DJI Digital FPV System`, `DJI Goggles`, `DJI Goggles RE`, `DJI O3 Air Unit`, `DJI O4 Air Unit Series` | Osmo Nano Vision Dock + five exact reviewed exclusions; DJI closure |
...
### DJI Ronin 4D / original Osmo Action — Wave 39

Wave 39 activates exactly `DJI Ronin 4D` and `Osmo Action`. DJI's official Ronin 4D support page and DJI Store establish TB50 Intelligent Battery and Inspire 2/Ronin 2 Battery Charging Hub compatibility with Ronin 4D. DJI's original Osmo Action specifications identify the 1300 mAh battery, and the official Download Center publishes the Osmo Action Charging Hub User Guide.
...
### Final DJI power review — Wave 41

Wave 41 closes the final six canonical DJI rows. `Osmo Nano` receives the exact `DJI Osmo Nano Multifunctional Vision Dock` handoff; DJI documents the Dock as the model-specific companion that extends operating time and provides charging/connection functions.

The five remaining rows are exact reviewed exclusions. `DJI Goggles` and `DJI Goggles RE` have integrated battery designs without a separately established model-specific replacement power accessory. `DJI O3 Air Unit` and `DJI O4 Air Unit Series` are externally powered components. `DJI Digital FPV System` is a multi-component system with separate power paths, so a single system-level battery/charger handoff is not asserted.
...
The deterministic Amazon handoff is `OM SYSTEM BLX-1 Lithium Ion Rechargeable Battery` for each of the three reviewed rows. No compatibility is inferred to `OM-5`, `OM-5 Mark II`, E-M series models, or any other OM SYSTEM row.

### BLH-1 — Wave 2

Wave 2 activates exactly `E-M1 Mark II`, `E-M1 Mark III`, and `E-M1X`. OM SYSTEM's official BLH-1 product page explicitly lists all three as compatible. The deterministic handoff is `OM SYSTEM BLH-1 Lithium Ion Rechargeable Battery`.

### LI-92B — Wave 3

Wave 3 activates exactly `TG-4`, `TG-5`, `TG-6`, and `TG-7`. OM SYSTEM's official LI-92B product information states that LI-92B works in LI-90B cameras and explicitly names TG-4 through TG-7. The deterministic handoff is `OM SYSTEM LI-92B Lithium Ion Rechargeable Battery`.
...
## GoPro camera review — Waves 1–4

### HERO13 Black — Wave 1

Wave 1 activates exactly `HERO13 Black`. GoPro's official Enduro for HERO13 Black page identifies the 1900 mAh battery as a replacement/spare for HERO13 Black and states that its connector is specific to HERO13 Black. The deterministic handoff is `GoPro Enduro Battery HERO13 Black`.

### HERO9–HERO12 Enduro — Wave 2

Wave 2 activates exactly `HERO9 Black`, `HERO10 Black`, `HERO11 Black`, and `HERO12 Black`. GoPro's official Enduro page explicitly lists all four models as compatible. The deterministic handoff is `GoPro Enduro Battery`.

### HERO5–HERO8 battery family — Wave 3

Wave 3 activates exactly `HERO5 Black`, `HERO6 Black`, `HERO7 Black`, `HERO8 Black`, and canonical `HERO 2018`. GoPro's official HERO8 Black Rechargeable Battery page lists HERO8 Black, HERO7 Black, HERO6 Black, and HERO (2018), and its compatibility note also covers HERO5 Black subject to a camera software update. The catalog's `HERO 2018` spelling is the normalized canonical form of GoPro's displayed `HERO (2018)`. The deterministic handoff is `GoPro HERO8 Black Rechargeable Battery`.

### MAX / MAX2 Enduro — Wave 4

Wave 4 activates exactly `MAX` and `MAX2` from their separate GoPro official Enduro battery pages. The deterministic handoffs are `GoPro Enduro Battery MAX` and `GoPro Enduro Battery MAX2`.
...
- `affiliate-dji-camera-accessories-wave1.js` through `affiliate-dji-camera-accessories-wave41.js` own reviewed DJI camera mappings.
...
- `affiliate-dji-camera-accessories-wave39.js` adds the exact Ronin 4D / Osmo Action battery-and-hub mappings.
...
- `affiliate-gopro-camera-accessories-wave1.js` adds the exact HERO13 Black Enduro mapping.
...
- `tests/dji-final-power-wave41.test.mjs` protects the final DJI closure boundary.
...
- `tests/gopro-hero13-accessory-wave1.test.mjs` protects the exact HERO13 Wave 1 boundary.
