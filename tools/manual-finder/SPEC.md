...
- The current camera state is **185 basic = 28 detail + 0 reviewed exclusions + 157 missing accessory detail** across 192 canonical camera-category records; 7 maker/index rows are non-actionable and excluded from the actionable denominator.
...
- The measured DJI state after Waves 1–5 is **96 basic = 14 detail + 0 reviewed exclusions + 82 missing accessory detail**.
...
- The measured remaining camera backlog is DJI 82, OM SYSTEM 37, GoPro 31, and Insta360 7. Exact missing models are emitted by `tests/camera-accessory-coverage.test.mjs` and summarized by `CAMERA_ACCESSORY_COVERAGE.md`.
...
- [x] DJI Waves 1–5 reconcile to **96 basic = 14 detail + 0 reviewed exclusions + 82 missing accessory detail**.
- [x] The catalog-wide camera audit reconciles to **185 basic = 28 detail + 0 reviewed exclusions + 157 missing accessory detail**, while seven maker/index rows remain explicitly non-actionable.
