(() => {
  ...
  const JSON_URL = `${base}/data/manuals.json?v=mf-wave1-20260912a`;
  ...
  function normalize(rows) {
    ...
    const normalized = (rows || []).filter(Boolean).map((x) => {
      ...
      return {
        ...
        sourceType: x.sourceType || "official",
        sourceLevel: x.sourceLevel || "",
        verifiedAt: x.verifiedAt || "",
        evidenceUrl: x.evidenceUrl || "",
        resolutionState: x.resolutionState || "",
        manualKind: x.manualKind || "",
        sharedTarget: Boolean(x.sharedTarget),
        linkReview: x.linkReview || "official entry"
      };
    }).filter((x) => {
      const key = x.id
        ? `id:${x.id}`
        : x.model
          ...
          : `base:${x.brand}|${x.category}|${x.manualUrl}`.toLowerCase();
    ...
    return normalized.sort((a, b) => {
      ...
      const am = a.model || a.nameEn;
      const bm = b.model || b.nameEn;
      return am.localeCompare(bm, "en", { numeric: true, sensitivity: "base" });
    });
  }
  function searchText(x) {
    return [
      x.brand, x.maker, x.model, x.family, x.nameJa, x.nameEn, x.category, x.country,
      x.note, x.noteJa, x.noteEn, x.hint, x.hintJa, x.hintEn,
      ...(x.aliases || []), ...(x.tags || [])
    ].join(" ").toLowerCase();
  }
  function lang(lang) {
  ...
  function render() {
    ...
    items.forEach((x) => {
      ...
      card.className = "card";
      const title = x.model ? `${x.maker} ${x.model}` : (S.lang === "ja" ? x.nameJa : x.nameEn);
      card.appendChild(txt("div", "card-title", title));
      ...
      if (x.verifiedAt) {
        const audit = x.sharedTarget
          ? t(`確認：公式共通ページ・${x.verifiedAt}`, `Checked: official shared page · ${x.verifiedAt}`)
          : t(`確認：公式ページ・${x.verifiedAt}`, `Checked: official page · ${x.verifiedAt}`);
        card.appendChild(txt("p", "card-audit", audit));
  ...
  async function initData() {
    ...
    try {
      ...
      if (typeof window.MANUALFINDER_BUILD_WAVE1 === "function") {
        waveRows = window.MANUALFINDER_BUILD_WAVE1();
      } else if (Array.isArray(window.MANUALFINDER_FULL_RECORDS)) {
        waveRows = window.MANUALFINDER_FULL_RECORDS;
      }
    } catch (_) {
      waveRows = [];
    }
    try {
      ...
      if (!r.ok) throw new Error(`manuals.json ${r.status}`);
      const data = await r.json();
      if (Array.isArray(data)) baseRows = data;
    } catch (_) {
      baseRows = [];
    }
    S.all = normalize([...baseRows, ...waveRows]);
    if (E.status) {
