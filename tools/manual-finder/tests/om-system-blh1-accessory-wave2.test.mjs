...
const wave = context.window.MANUALFINDER_OM_SYSTEM_CAMERA_ACCESSORY_WAVE2_LEDGER;
const models = ['E-M1 Mark II', 'E-M1 Mark III', 'E-M1X'];
...
for (const model of models) {
  ...
  assert.equal(offers.length, 1, `${model} must expose exactly the reviewed BLH-1 handoff`);
  assert.equal(offers[0].kind, 'camera_battery_search');
  assert.equal(offers[0].query, 'OM SYSTEM BLH-1 Lithium Ion Rechargeable Battery');
  assert.equal(offers[0].url, 'https://www.amazon.co.jp/s?k=OM+SYSTEM+BLH-1+Lithium+Ion+Rechargeable+Battery&tag=nicheworks09-22');
  assert.equal(offers[0].sourceUrl, 'https://explore.omsystem.com/us/en/blh-1-li-ion-rechargeable-battery');
...
for (const args of [
  ...
  assert.deepEqual(Array.from(config.getAccessoryOffers(args)), [], `unreviewed BLH-1 mapping must fail closed: ${JSON.stringify(args)}`);
}

console.log('ManualFinder OM SYSTEM BLH-1 camera accessory Wave 2 passed.');
