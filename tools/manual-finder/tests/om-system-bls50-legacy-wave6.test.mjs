...
const waveRows = context.window.MANUALFINDER_OM_SYSTEM_CAMERA_ACCESSORY_WAVE6_LEDGER;
const models = ["E-M10","E-M10 Mark II","E-P1","E-P2","E-P3","E-PL1","E-PL2","E-PL3","E-PL5","E-PL6","E-PL7","E-PL8","E-PL9","E-PM1","E-PM2"];
assert.ok(config);
...
for (const model of models) {
  ...
  assert.equal(offers.length, 1, `${model} must expose exactly one reviewed BLS-50 battery handoff`);
...
assert.ok(runtimeSource.includes('affiliate-om-system-camera-accessories-wave6.js'));
assert.ok(runtimeSource.includes('MANUALFINDER_OM_SYSTEM_CAMERA_ACCESSORY_WAVE6_LEDGER'));

console.log('ManualFinder OM SYSTEM BLS-50 camera accessory Wave 6 passed.');
