...
const wave = context.window.MANUALFINDER_DJI_CAMERA_ACCESSORY_WAVE13_LEDGER;
const merged = context.window.MANUALFINDER_CAMERA_ACCESSORY_LEDGER;
assert.ok(config);
assert.ok(Array.isArray(wave));
assert.ok(Array.isArray(merged));
assert.equal(wave.length, 1, 'DJI Mavic Pro Wave 13 must contain exactly one reviewed model');
assert.equal(merged.length, 40, 'Nikon 14 + DJI 26 should produce forty camera detail rows');
assert.equal(config.cameraAccessories.length, 40);
...
assert.equal(row.model, 'Mavic Pro');
...
const offers = Array.from(config.getAccessoryOffers({ maker: 'DJI', model: 'Mavic Pro', category: 'カメラ・映像' }));
assert.equal(offers.length, 2, 'Mavic Pro must expose exactly one reviewed battery and one charging-hub handoff');
assert.deepEqual(offers.map((offer) => offer.kind), ['camera_battery_search', 'camera_charger_search']);
assert.deepEqual(offers.map((offer) => offer.query), ['Mavic Pro Intelligent Flight Battery', 'Mavic Pro Battery Charging Hub']);
assert.equal(offers[0].url, 'https://www.amazon.co.jp/s?k=Mavic+Pro+Intelligent+Flight+Battery&tag=nicheworks09-22');
...
for (const args of [
  ...
  { maker: 'Nikon', model: 'Mavic Pro', category: 'カメラ・映像' },
  { maker: 'DJI', model: 'Mavic Pro', category: 'その他' },
  ...
  if (args.model === 'Mavic 2' || args.model === 'Mavic Air') {
    assert.equal(actual.length, 2, `${args.model} must retain its separately reviewed existing handoffs`);
    assert.ok(actual.every((offer) => offer.sourceUrl !== batterySource || args.model === 'Mavic Air'), 'Wave 13 evidence must not replace an existing Mavic-family contract');
  } else {
    assert.deepEqual(actual, [], `unreviewed Mavic Pro mapping must fail closed: ${JSON.stringify(args)}`);
  }
}

const runtimeSource = fs.readFileSync(new URL('../affiliate-runtime.js', import.meta.url), 'utf8');
assert.ok(runtimeSource.includes('affiliate-dji-camera-accessories-wave13.js'), 'runtime must load DJI camera accessory Wave 13 before rendering');
assert.ok(runtimeSource.includes('MANUALFINDER_DJI_CAMERA_ACCESSORY_WAVE13_LEDGER'), 'runtime must wait for the DJI Wave 13 ledger');

console.log('ManualFinder DJI Mavic Pro camera accessory Wave 13 passed.');
