...
assert.equal(config.officePrinterConsumables.length, 68, '63 prior office mappings plus five Wave 6 rows');
assert.equal(config.printerConsumables.length, 93, '88 prior printer mappings plus five Wave 6 rows');
...
for (const [label, html, prefix] of [['JA', jaHtml, './'], ['EN', enHtml, '../']]) {
  ...
  assert.ok(wave45Index >= 0 && wave6Index > wave45Index, `${label} must load Wave 6 after the earlier KYOCERA supplemental bundle`);
  assert.ok(fujiIndex > wave6Index && runtimeIndex > fujiIndex, `${label} must compose Wave 6 before FUJIFILM and the runtime`);
...
assert.equal(context.window.MANUALFINDER_AFFILIATE_CONFIG.officePrinterConsumables.length, 68, 'Wave 6 must be idempotent when loaded twice');
