...
const wave = context.window.MANUALFINDER_DJI_CAMERA_ACCESSORY_WAVE5_LEDGER;
const merged = context.window.MANUALFINDER_CAMERA_ACCESSORY_LEDGER;
assert.ok(config);
assert.ok(Array.isArray(wave));
assert.ok(Array.isArray(merged));
assert.equal(wave.length, 2, 'DJI Air 2S/Mavic Air 2 Wave 5 must contain exactly two reviewed models');
assert.equal(merged.length, 28, 'Nikon 14 + DJI 14 should produce twenty-eight camera detail rows');
assert.equal(config.cameraAccessories.length, 28);

const models = ['DJI Air 2S', 'Mavic Air 2'];
assert.deepEqual(Array.from(wave, (row) => row.model).sort(), [...models].sort());

const batterySource = 'https://store.dji.com/product/mavic-air-2-intelligent-flight-battery';
const hubSource = 'https://store.dji.com/uk/product/mavic-air-2-battery-charging-hub';
const batteryQuery = 'DJI Mavic Air 2 Intelligent Flight Battery';
const hubQuery = 'DJI Mavic Air 2 Battery Charging Hub';
...
for (const row of wave) {
  ...
  assert.equal(offers[0].url, 'https://www.amazon.co.jp/s?k=DJI+Mavic+Air+2+Intelligent+Flight+Battery&tag=nicheworks09-22');
  assert.equal(offers[1].url, 'https://www.amazon.co.jp/s?k=DJI+Mavic+Air+2+Battery+Charging+Hub&tag=nicheworks09-22');
...
for (const args of [
  ...
  { maker: 'Nikon', model: 'DJI Air 2S', category: 'カメラ・映像' },
  { maker: 'DJI', model: 'DJI Air 2S', category: 'その他' },
  { maker: 'DJI', model: 'Mavic Air', category: 'カメラ・映像' },
  { maker: 'DJI', model: 'Mavic Air 2S', category: 'カメラ・映像' },
  { maker: 'DJI', model: 'DJI Mini 2', category: 'カメラ・映像' },
  { maker: 'DJI', model: '', category: 'カメラ・映像' }
]) {
  assert.deepEqual(Array.from(config.getAccessoryOffers(args)), [], `unreviewed DJI Air 2 mapping must fail closed: ${JSON.stringify(args)}`);
}

const runtimeSource = fs.readFileSync(new URL('../affiliate-runtime.js', import.meta.url), 'utf8');
assert.ok(runtimeSource.includes('affiliate-dji-camera-accessories-wave5.js'), 'runtime must load DJI camera accessory Wave 5 before rendering');
assert.ok(runtimeSource.includes('MANUALFINDER_DJI_CAMERA_ACCESSORY_WAVE5_LEDGER'), 'runtime must wait for the DJI Wave 5 ledger');

console.log('ManualFinder DJI Air 2S/Mavic Air 2 camera accessory Wave 5 passed.');
