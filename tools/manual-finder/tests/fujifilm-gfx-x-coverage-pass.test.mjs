...
assert.equal(validation.summary.reviewedCount,56);
...
assert.equal(validation.summary.states.direct,52);
assert.equal(validation.summary.states.shared,4);
assert.equal(validation.summary.states.support_only,0);
assert.equal(validation.summary.states.held,0);
assert.equal(validation.summary.reconciliation,'56 = 52 direct + 4 shared + 0 support_only + 0 held');
assert.deepEqual(new Set(pass.review.secondaryDiscovery.resolvedDirect),new Set(['GFX 50S II','X-E2']));
assert.deepEqual(new Set(pass.review.secondaryDiscovery.resolvedShared),new Set(['GFX 50S','GFX 50R','X-S20','X half']));
assert.deepEqual(pass.review.secondaryDiscovery.unresolved,[]);
