(function () {
  ...
  function sampleMarkup(record, large) {
    ...
    switch (record.sampleType) {
      ...
      case 'tooltip': core = `<button class="mini-btn tooltip-anchor" data-tooltip>${p('Hover/focus me', 'ホバー/フォーカス')}</button><span class="sample-tooltip">${p('Real tooltip', 'ツールチップ')}</span>`; break;
      case 'popover': core = `<button class="mini-btn" data-sample-pop>${p('Toggle popover', 'ポップオーバー')}</button><div class="sample-popover" data-popover>${p('Anchored popover panel', 'アンカー付きパネル')}</div>`; break;
      case 'toast': core = `<button class="mini-btn" data-sample-toast>${p('Show toast', 'トースト表示')}</button><div class="sample-toast" data-toast>${p('Saved successfully', '保存しました')}</div>`; break;
      ...
      case 'drawer': core = `<button class="mini-btn" data-sample-open="drawer">${p('Open drawer', 'ドロワー開く')}</button><aside class="sample-drawer" data-sample-drawer><button data-sample-close>×</button><p>Menu A</p><p>Menu B</p></aside>`; break;
      ...
      case 'hamburger': core = `<button class="mini-btn" data-sample-open="ham">☰</button><div class="sample-ham" data-sample-ham><a href="#">Docs</a><a href="#">API</a></div>`; break;
      ...
      case 'text-input': core = `<label>${p('Email', 'メール')}<input type="email" placeholder="hello@example.com"></label>`; break;
      case 'textarea': core = `<label>${p('Message', 'メッセージ')}<textarea rows="2">${p('Draft text', '下書きテキスト')}</textarea></label>`; break;
      case 'combobox': core = `<label>${p('Search tag', 'タグ検索')}<input list="cb-${record.id}" placeholder="UI"><datalist id="cb-${record.id}"><option>UI</option><option>UX</option><option>Design</option></datalist></label>`; break;
      case 'date-picker': core = `<label>${p('Date', '日付')}<input type="date"></label>`; break;
      case 'validation': core = `<label>${p('Username', 'ユーザー名')}<input value="ab"><small>${p('Must be at least 3 characters.', '3文字以上で入力してください。')}</small></label>`; break;
      ...
      case 'testimonial': core = `<blockquote class="sample-testimonial">“${p('Great workflow and clear UX.', '分かりやすく実務で使いやすい。')}”<cite>— Team</cite></blockquote>`; break;
