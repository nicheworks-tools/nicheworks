(() => {
  ...
  const bulletBlock = (items, fallbackText) => items.length ? items.map((item) => `- ${item}`).join("\n") : `- ${fallbackText}`;
  ...
  const buildJobPost = (lang) => {
    ...
    if (lang === "ja") {
      return [
        "【募集職種】", role, "",
        "【契約形態 / 雇用形態】", contractType, "",
        "【報酬】", compensation, "",
        "【稼働時間】", workload, "",
        "【業務内容】", bulletBlock(responsibilities, "業務内容、成果物、確認方法を明確にして追記してください"), "",
        "【必須要件】", bulletBlock(requirements, "類似業務の経験、稼働条件、必要スキルを追記してください"), "",
        "【歓迎要件】", bulletBlock(preferred, "コミュニケーションの速さ、改善提案、自走力など"), "",
        "【勤務条件・補足】", bulletBlock(conditions, "定例頻度、連絡手段、納品形式などを追記してください"), "",
        "【勤務地 / 働き方】", location, "",
        "【選考プロセス】", bulletBlock(process, "書類選考 → 面談 → 必要に応じてトライアル"), "",
        "【応募方法】", `- ${applyMethod}`, "",
        "【公開前チェック】", "- 労働条件、契約形態、最低賃金、差別的表現、求人媒体ルールを確認してください。"
      ].join("\n");
    }

    return [
      "[Role]", role, "",
      "[Contract / Employment Type]", contractType, "",
      "[Compensation]", compensation, "",
      "[Expected Workload]", workload, "",
      "[Responsibilities]", bulletBlock(responsibilities, "Add concrete tasks, deliverables, and review methods"), "",
      "[Required Qualifications]", bulletBlock(requirements, "Add relevant experience, availability, and required skills"), "",
      "[Preferred Qualifications]", bulletBlock(preferred, "Fast communication, improvement suggestions, proactive ownership"), "",
      "[Working Conditions / Notes]", bulletBlock(conditions, "Add meeting cadence, communication channel, and delivery format"), "",
      "[Location / Remote]", location, "",
      "[Hiring Process]", bulletBlock(process, "Application review → Interview → Trial task if needed"), "",
      "[How to Apply]", `- ${applyMethod}`, "",
      "[Pre-publish Check]", "- Review labor conditions, contract type, minimum wage rules, discriminatory wording, and job board policies."
    ].join("\n");
  };
  ...
  const buildSheetColumns = (lang) => (lang === "ja" ? ["応募日","氏名","メール","ポートフォリオ","類似実績","希望報酬","稼働可能時間","契約形態確認","トライアル可否","コミュニケーション手段","ステータス","次アクション","メモ"] : ["Application Date","Name","Email","Portfolio","Relevant Experience","Expected Compensation","Availability","Contract Type Check","Trial Task Availability","Communication Channel","Status","Next Action","Notes"]).join(",");
  ...
  const refresh = () => {
    document.getElementById("outputJa").textContent = buildKit("ja");
    document.getElementById("outputEn").textContent = buildKit("en");
  };
  const getCurrentOutput = () => document.getElementById(currentLang() === "ja" ? "outputJa" : "outputEn").textContent.trim();
  ...
  const initTool = () => {
    ...
    document.getElementById("generateBtn").addEventListener("click", () => { refresh(); toast(currentLang() === "ja" ? "求人キットを生成しました。" : "Generated the job kit."); });
    document.getElementById("copyBtn").addEventListener("click", async () => {
      refresh();
      const ok = await copyToClipboard(getCurrentOutput());
      toast(ok ? (currentLang() === "ja" ? "コピーしました。" : "Copied.") : (currentLang() === "ja" ? "コピーに失敗しました。" : "Copy failed."));
    });
    document.getElementById("saveTxtBtn").addEventListener("click", () => { refresh(); downloadText("niche-job-starter-kit.txt", getCurrentOutput()); toast(currentLang() === "ja" ? "TXTを保存しました。" : "Saved TXT."); });
    document.getElementById("copyCsvBtn").addEventListener("click", async () => {
      const ok = await copyToClipboard(buildSheetColumns(currentLang()));
      extraOutputJa.textContent = buildSheetColumns("ja");
      extraOutputEn.textContent = buildSheetColumns("en");
      toast(ok ? (currentLang() === "ja" ? "CSV列をコピーしました。" : "Copied CSV columns.") : (currentLang() === "ja" ? "CSV列のコピーに失敗しました。" : "Failed to copy CSV columns."));
    });
    document.getElementById("saveCsvBtn").addEventListener("click", () => { const lang = currentLang(); downloadText("candidate-sheet-columns.csv", buildCsvDownload(lang), "text/csv;charset=utf-8"); toast(lang === "ja" ? "CSVを保存しました。" : "Saved CSV."); });
    document.getElementById("variantBtn").addEventListener("click", () => { extraOutputJa.textContent = buildVariants("ja"); extraOutputEn.textContent = buildVariants("en"); toast(currentLang() === "ja" ? "バリエーションを生成しました。" : "Generated variants."); });
