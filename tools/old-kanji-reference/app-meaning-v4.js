(() => {
  ...
  const messages = {
    ja: { loading: "辞書データを読み込み中です…", loadError: "辞書データの読み込みに失敗しました。", copiedOld: "旧字体をコピーしました", copiedNew: "現代表記をコピーしました", noMatch: "該当する旧字体が見つかりませんでした。別の漢字・読み・新字体で検索してください。", showing: "表示中", searchResults: "検索結果", total: "全" },
    en: { loading: "Loading dictionary…", loadError: "Failed to load dictionary.", copiedOld: "Copied old form", copiedNew: "Copied modern form", noMatch: "No matching entries found. Try another kanji, reading, or modern form.", showing: "Showing", searchResults: "Search results", total: "total" }
  };

  const filters = [
    { id: "all", ja: "すべて", en: "All" },
    { id: "verified", ja: "確認済みのみ", en: "Verified only" },
    { id: "hasMeaning", ja: "読み・意味あり", en: "With meaning" },
  ...
  function getCodePoint(char){ return `U+${char.codePointAt(0).toString(16).toUpperCase().padStart(4, "0")}`; }
  ...
  function hasMeaning(entry){ return Boolean(entry.readingJa || entry.readingEn || entry.meaningJa || entry.meaningEn); }
  ...
  function switchLang(lang){
    ...
    if (searchInput) searchInput.placeholder = currentLang === "en" ? (searchInput.dataset.placeholderEn || "Search old form, modern form, reading, or meaning") : (searchInput.dataset.placeholderJa || "旧字体・現代表記・読み・意味で検索できます");
  }
  ...
  async function loadData(){
    const [dict, meta, extra2, extra3] = await Promise.all([
      fetchJson("./dict.json"),
      fetchJson("./meta.json?v=20260503-okj-meta-3"),
      fetchJson("./meta-extra-2.json?v=20260503-okj-extra-3").catch(() => ({ entries: {} })),
      fetchJson("./meta-extra-3.json?v=20260518-okj-extra-3").catch(() => ({ entries: {} }))
    ]);
    metaCache = { popularOrder: meta.popularOrder || [], entries: Object.assign({}, meta.entries || {}, extra2.entries || {}, extra3.entries || {}) };
    return dict;
  }
  ...
  function buildEntries(dict){
    ...
    entriesCache = Object.entries(dict.old_to_new || {}).map(([oldChar, newChar]) => {
      ...
      return { oldChar, newText, oldCode: getCodePoint(oldChar), newCode: getCodePoint(newText[0] || newText), block: getBlockName(oldChar), category: getCategory(oldChar), verified: Boolean(meta.verified), readingJa: meta.readingJa || "", readingEn: meta.readingEn || "", meaningJa: meta.meaningJa || "", meaningEn: meta.meaningEn || "", usageJa: meta.usageJa || "", usageEn: meta.usageEn || "" };
    });
  ...
  function createEntryCard(entry, compact){
    ...
    const codes = document.createElement("p"); codes.className = "kanji-codepoints"; codes.innerHTML = `<span data-i18n="ja">Unicode：旧字 ${entry.oldCode} / 新字 ${entry.newCode}</span><span data-i18n="en">Unicode: old ${entry.oldCode} / modern ${entry.newCode}</span>`; card.appendChild(codes);
  ...
  function getFilteredEntries(){ const q = currentQuery.trim().toLowerCase(); const popularSet = getPopularSet(); return entriesCache.filter(entry => { const matchesFilter = currentFilter === "all" || (currentFilter === "verified" && entry.verified) || (currentFilter === "hasMeaning" && hasMeaning(entry)) || entry.category === currentFilter || (currentFilter === "popular" && popularSet.has(entry.oldChar)); const haystack = `${entry.oldChar} ${entry.newText} ${entry.readingJa} ${entry.readingEn} ${entry.meaningJa} ${entry.meaningEn} ${entry.usageJa} ${entry.usageEn} ${entry.oldCode} ${entry.newCode} ${labelForCategory(entry.category)}`.toLowerCase(); return matchesFilter && (!q || haystack.includes(q)); }); }
  function renderReverseSummary(entries){
    let panel = document.getElementById("reverseSummary");
    const groupWrapper = document.querySelector(".group-wrapper");
    if (!groupWrapper) return;
    if (!panel) { panel = document.createElement("div"); panel.id = "reverseSummary"; panel.className = "reverse-summary"; groupWrapper.insertBefore(panel, document.getElementById("groupContainer")); }
    const q = currentQuery.trim();
    const exact = q ? entriesCache.filter(e => e.newText === q || e.oldChar === q) : [];
    const byNew = exact.length ? new Map() : null;
    if (!byNew) { panel.hidden = true; panel.innerHTML = ""; return; }
    exact.forEach(e => { if (!byNew.has(e.newText)) byNew.set(e.newText, []); byNew.get(e.newText).push(e.oldChar); });
    panel.hidden = false;
    panel.innerHTML = Array.from(byNew.entries()).map(([modern, olds]) => `<p><span data-i18n="ja">逆引き：${modern} ← ${olds.join(" / ")}</span><span data-i18n="en">Reverse lookup: ${modern} ← ${olds.join(" / ")}</span></p>`).join("");
  }
  function renderGroups(entries){
    const container = document.getElementById("groupContainer"); const emptyMessage = document.getElementById("emptyMessage"); if (!container || !emptyMessage) return; container.innerHTML = ""; emptyMessage.hidden = entries.length > 0; renderReverseSummary(entries);
