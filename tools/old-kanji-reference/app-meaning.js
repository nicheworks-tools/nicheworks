(() => {
  ...
  function getPopularOrder() {
    return Array.isArray(metaCache.popularOrder) && metaCache.popularOrder.length ? metaCache.popularOrder : fallbackPopularOrder;
  }
  function getPopularSet() {
    return new Set(getPopularOrder());
  }
  ...
  function getMeta(oldChar) {
    return (metaCache.entries && metaCache.entries[oldChar]) || {};
  }
  function getCategory(oldChar) {
    const meta = getMeta(oldChar);
    if (meta.category) return meta.category;
    if (getPopularSet().has(oldChar)) return "popular";
  ...
  async function loadData() {
    const [dict, meta] = await Promise.all([
      fetchJson("./dict.json"),
      fetchJson("./meta.json?v=20260503-okj-meta-1")
    ]);
    metaCache = meta || { popularOrder: [], entries: {} };
    return dict;
  }
  ...
  function createEntryCard(entry, compact) {
    ...
    const verifiedJa = entry.verified ? " / 確認済み" : "";
    const verifiedEn = entry.verified ? " / verified" : "";
    meta.innerHTML = `<span data-i18n="ja">分類：${labelForCategory(entry.category)}${verifiedJa}</span><span data-i18n="en">Category: ${labelForCategory(entry.category)}${verifiedEn}</span>`;
  ...
  function renderPopular() {
    ...
    if (!container) return;
    container.innerHTML = "";
    const popularSet = getPopularSet();
    entriesCache.filter(entry => popularSet.has(entry.oldChar) && entry.verified).slice(0, 26).forEach(entry => container.appendChild(createEntryCard(entry, true)));
  }
  function getFilteredEntries() {
    const q = currentQuery.trim().toLowerCase();
    const popularSet = getPopularSet();
    return entriesCache.filter(entry => {
      const matchesFilter = currentFilter === "all" || entry.category === currentFilter || (currentFilter === "popular" && popularSet.has(entry.oldChar));
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    loadData().then(dict => { setCounts(dict); buildEntries(dict); renderAll(); }).catch(() => setStatusText(messages[currentLang].loadError));
  });
})();
