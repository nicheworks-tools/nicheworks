...
const ledger = JSON.parse(fs.readFileSync(path.join(DATA, 'image-wave2-sources-v2.3.json'), 'utf8'));
const resolutions = JSON.parse(fs.readFileSync(path.join(DATA, 'image-identity-resolutions-v2.3.json'), 'utf8'));
const errors = [];
...
if (ledger.schema !== 'cta-image-wave2-sources-v2.3') errors.push('source ledger schema marker is invalid');
...
if (resolutions.schema !== 'cta-image-identity-resolutions-v2.3') errors.push('identity resolution ledger schema marker is invalid');

const corpusIds = loadCorpusIds();
const resolvedIds = new Set(array(resolutions.resolutions).filter((row) => row?.state === 'resolved_existing').map((row) => text(row.entry_id)).filter(Boolean));
...
for (const item of array(ledger.items)) {
  ...
  if (!resolvedIds.has(id)) errors.push(`${id}: Wave 2 source must be backed by a resolved_existing identity record`);
  ...
  if (sourceUrl && sourceUrl.hostname !== 'upload.wikimedia.org') errors.push(`${id}: Wave 2 binary source must be upload.wikimedia.org`);
  if (sourcePage && sourcePage.hostname !== 'commons.wikimedia.org') errors.push(`${id}: Wave 2 source page must be commons.wikimedia.org`);
  if (sourceUrl) {
  ...
  if (item.subject_match !== 'matched') errors.push(`${id}: subject_match must be matched before Wave 2 acquisition`);
...
if (errors.length) {
  console.error('Construction Tools Atlas Wave 2 image source ledger: FAIL');
  errors.forEach((error) => console.error(`- ${error}`));
  process.exit(1);
}

console.log('Construction Tools Atlas Wave 2 image source ledger: PASS');
console.log(`- reviewed acquisition sources: ${ledger.items.length}`);
console.log(`- canonical IDs represented: ${seenIds.size}`);
console.log('- every Wave 2 source is backed by an explicit resolved_existing legacy-to-canonical identity decision');
