...
const WAVE4_PATH = path.join(DATA, 'image-wave4-sources-v2.3.json');
...
function validateItem(item, seen) {
  ...
  if (!/^[a-z0-9_]+$/.test(id)) fail(`Invalid Wave 4 entry_id: ${id || '<missing>'}`);
  ...
  if (item.subject_match !== 'matched') fail(`${id}: Wave 4 build requires subject_match=matched`);
  if (!['reviewed', 'verified'].includes(item.review_state)) fail(`${id}: Wave 4 build requires reviewed/verified source`);
...
async function main() {
  const wave4 = JSON.parse(await fs.readFile(WAVE4_PATH, 'utf8'));
  if (wave4.schema !== 'cta-image-wave4-sources-v2.3') fail('Unexpected Wave 4 source ledger schema.');
  if (!Array.isArray(wave4.items) || wave4.items.length === 0) fail('Wave 4 source ledger must contain items.');
  ...
  await fs.writeFile(WAVE4_PATH, `${JSON.stringify(wave4, null, 2)}\n`, 'utf8');
  ...
  console.log(`Wave 4A image build complete: ${wave4.items.length} canonical entries`);
}

main().catch((error) => {
  console.error(`Construction Tools Atlas Wave 4A image build: FAIL\n${error.stack || error.message || error}`);
  process.exit(1);
});
