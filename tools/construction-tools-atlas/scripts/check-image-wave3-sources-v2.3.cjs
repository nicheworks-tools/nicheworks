...
const ledger = JSON.parse(fs.readFileSync(path.join(DATA, 'image-wave3-sources-v2.3.json'), 'utf8'));
...
if (ledger.schema !== 'cta-image-wave3-sources-v2.3') errors.push('source ledger schema marker is invalid');
...
for (const item of array(ledger.items)) {
  ...
  if (!/^q017_[a-z0-9_]+$/.test(id)) errors.push(`${id}: Wave 3A entry_id must be an active q017 canonical ID`);
  ...
  const materialized = hash || assetFlags.some(Boolean) || Boolean(canonical);
  ...
  if (!/^[a-f0-9]{40}$/.test(hash)) errors.push(`${id}: materialized Wave 3 source requires pinned source_sha1`);
  if (!assetFlags.every(Boolean)) errors.push(`${id}: materialized Wave 3 source requires retained source + primary.webp + thumb.webp`);
  if (!canonical) {
    errors.push(`${id}: materialized Wave 3 source must be promoted into the canonical registry`);
    continue;
  }
  promoted += 1;
  if (canonical.image_state !== 'reviewed' && canonical.image_state !== 'verified') errors.push(`${id}: promoted registry image must be reviewed or verified`);
  ...
  for (const [field, value] of Object.entries(expectedSource(item))) {
    if (text(canonical.source?.[field]) !== value) errors.push(`${id}: registry source.${field} must exactly match Wave 3 source ledger`);
  }
...
if (promoted > 0 && !markdown.includes(`\`${text(ledger.version)}\``)) {
  errors.push(`images/ATTRIBUTION.md is missing Wave 3 source ledger version ${ledger.version}`);
}

if (errors.length) {
  console.error('Construction Tools Atlas Wave 3 image source contract: FAIL');
  errors.forEach((error) => console.error(`- ${error}`));
  process.exit(1);
}

console.log('Construction Tools Atlas Wave 3 image source contract: PASS');
console.log(`- reviewed Wave 3A sources: ${ledger.items.length}`);
console.log(`- staged acquisition rows: ${staged}`);
console.log(`- fully materialized/promoted rows: ${promoted}`);
console.log('- q017 subject identity, local raster assets, registry provenance, and attribution are gated once materialized');
