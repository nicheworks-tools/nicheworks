...
function sourceQuality(row) {
  ...
  const detail = bilingualPair(row, 'detail', 'nj', 'ne', 'detail_ja', 'detail_en');
  const bullets = bilingualArrays(row, 'bullets', 'bj', 'be', 'bullets_ja', 'bullets_en');
  const examples = bilingualArrays(row, 'examples', 'ej', 'ee', 'examples_ja', 'examples_en');
  ...
  return { identity: identity.ok, definition: definition.ok, detail: detail.ok, bullets: bullets.ok, examples: examples.ok, aliases: aliases.ok, categories, tasks, fallbackFields, status };
}
...
async function runLoader() {
  ...
  vm.runInNewContext(source, { window: windowObject, document: documentStub, console: { info() {}, warn() {}, error: console.error }, Set }, { filename: 'quality-loader.js' });
  return windowObject.CTA_DATA_LOADER.loadEntries();
}
...
async function compute() {
  ...
  for (const publicEntry of runtime) {
    ...
    const q = sourceQuality(source.row);
    ...
    statuses.push({ id, status: q.status, fallback: q.fallbackFields.join(',') || '-' });
    if (samples[q.status].length < 25) samples[q.status].push({ id, source: source.source, fallback_fields: q.fallbackFields });
  }

  const sortedStatuses = statuses.sort((a, b) => a.id.localeCompare(b.id, 'en'));
  return {
    schema: 'cta-public-content-quality-v2.3',
    version: '2026-09-15-content-inventory-1',
    policy: {
      purpose: 'Measure source-backed core content separately from runtime generic fallback copy.',
    ...
    hashes: {
      content_status_sha256: hashLines(sortedStatuses.map((row) => `${row.id}\t${row.status}\t${row.fallback}`)),
