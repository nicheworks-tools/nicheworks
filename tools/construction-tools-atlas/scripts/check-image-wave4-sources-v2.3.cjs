...
const ledger = JSON.parse(fs.readFileSync(path.join(DATA, 'image-wave4-sources-v2.3.json'), 'utf8'));
...
if (ledger.schema !== 'cta-image-wave4-sources-v2.3') errors.push('source ledger schema marker is invalid');
...
for (const item of array(ledger.items)) {
  ...
  if (!hash) errors.push(`${id}: materialized Wave 4 source requires pinned source_sha1`);
  if (!assetFlags.every(Boolean)) errors.push(`${id}: materialized Wave 4 source requires retained source + primary.webp + thumb.webp`);
  if (!canonical) {
    errors.push(`${id}: materialized Wave 4 source must be promoted into the canonical registry`);
    continue;
  }
  promoted += 1;
  ...
  for (const [field, value] of Object.entries(expectedSource(item))) {
    if (text(canonical.source?.[field]) !== value) errors.push(`${id}: registry source.${field} must exactly match Wave 4 source ledger`);
  }
...
if (promoted > 0 && !markdown.includes(`\`${text(ledger.version)}\``)) {
  errors.push(`images/ATTRIBUTION.md is missing Wave 4 source ledger version ${ledger.version}`);
}

if (errors.length) {
  console.error('Construction Tools Atlas Wave 4 image source contract: FAIL');
  errors.forEach((error) => console.error(`- ${error}`));
  process.exit(1);
}

console.log('Construction Tools Atlas Wave 4 image source contract: PASS');
console.log(`- reviewed Wave 4A sources: ${ledger.items.length}`);
