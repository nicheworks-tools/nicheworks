...
const WAVE3_PATH = path.join(DATA, 'image-wave3-sources-v2.3.json');
...
function validateItem(item, seen) {
  ...
  if (!/^q017_[a-z0-9_]+$/.test(id)) fail(`Invalid Wave 3 q017 entry_id: ${id || '<missing>'}`);
  if (seen.has(id)) fail(`${id}: duplicate entry_id`);
  seen.add(id);
  for (const field of ['source_url', 'source_page', 'source_filename', 'license', 'license_url', 'author', 'attribution', 'review_note']) {
  ...
  if (item.subject_match !== 'matched') fail(`${id}: Wave 3 build requires subject_match=matched`);
  if (!['reviewed', 'verified'].includes(item.review_state)) fail(`${id}: Wave 3 build requires reviewed/verified source`);
...
async function main() {
  const [wave1, wave2, wave3] = await Promise.all([
    fs.readFile(WAVE1_PATH, 'utf8').then(JSON.parse),
    fs.readFile(WAVE2_PATH, 'utf8').then(JSON.parse),
    fs.readFile(WAVE3_PATH, 'utf8').then(JSON.parse)
  ]);
  if (wave1.schema !== 'cta-image-wave1-sources-v2.3') fail('Unexpected Wave 1 source ledger schema.');
  if (wave2.schema !== 'cta-image-wave2-sources-v2.3') fail('Unexpected Wave 2 source ledger schema.');
  if (wave3.schema !== 'cta-image-wave3-sources-v2.3') fail('Unexpected Wave 3 source ledger schema.');
  if (!Array.isArray(wave3.items) || wave3.items.length === 0) fail('Wave 3 source ledger must contain items.');
  ...
  for (const item of wave3.items) {
    const id = validateItem(item, seen);
    const dir = path.join(IMAGE_ROOT, id);
    await fs.mkdir(dir, { recursive: true });
    const buffer = await download(item);
    const hash = sha1(buffer);
    if (item.source_sha1 && hash !== item.source_sha1) fail(`${id}: SHA-1 mismatch; expected ${item.source_sha1}, got ${hash}`);
  ...
  await fs.writeFile(WAVE3_PATH, `${JSON.stringify(wave3, null, 2)}\n`, 'utf8');
  await fs.writeFile(path.join(IMAGE_ROOT, 'ATTRIBUTION.md'), attributionMarkdown([wave1, wave2, wave3], actualHashes), 'utf8');
  console.log(`Wave 3A image build complete: ${wave3.items.length} canonical entries`);
}

main().catch((error) => {
  console.error(`Construction Tools Atlas Wave 3A image build: FAIL\n${error.stack || error.message || error}`);
  process.exit(1);
});
