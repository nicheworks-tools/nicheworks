...
function normalizeProductId(value) {
  ...
  if (!productId) throw new TypeError("Logistics productId is required before product-scoped activation can be staged.");
...
function normalizeFeatureMap(input) {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    throw new TypeError("A complete Logistics featureMap is required.");
  }

  const normalized = Object.create(null);
  const seen = new Set();
  for (const operation of LOGISTICS_PRO_OPERATIONS) {
    const featureId = String(input[operation] || "").trim();
    if (!featureId) throw new TypeError(`Missing feature ID for Logistics operation: ${operation}`);
  ...
  for (const key of Object.keys(input)) {
    if (!LOGISTICS_PRO_OPERATIONS.includes(key)) {
      throw new TypeError(`Unknown Logistics paid operation: ${key}`);
    }
  }
...
function inactiveState(productId, featureMap, reason = "not_verified") {
  return Object.freeze({
    ...
    operations: Object.freeze(Object.fromEntries(
      LOGISTICS_PRO_OPERATIONS.map((operation) => [operation, false])
    )),
    featureMap
  });
}
...
function verifiedState(productId, featureMap, response) {
  ...
  return Object.freeze({
    ...
    operations: Object.freeze(Object.fromEntries(
      LOGISTICS_PRO_OPERATIONS.map((operation) => [operation, featureSet.has(featureMap[operation])])
    )),
    featureMap
  });
}
...
export function createLogisticsProductScopedController({ entitlementClient, productId, featureMap } = {}) {
  ...
  const can = (operation) => {
    if (!LOGISTICS_PRO_OPERATIONS.includes(operation)) {
      throw new TypeError(`Unknown Logistics paid operation: ${operation}`);
    }
