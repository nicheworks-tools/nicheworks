...
const EXPECTED_WAVE24 = Object.freeze({ triethoxycaprylylsilane:'binder', 'p-anisic acid':'fragrance', 'polyquaternium-39':'film former', 'polyquaternium-53':'hair conditioning', 'ppg-5-ceteth-20':'emulsifier', 'snail secretion filtrate':'skin conditioning', 'synthetic beeswax':'viscosity adjuster' });
const EXPECTED_WAVE24_LEGACY_CATEGORIES = Object.freeze({ triethoxycaprylylsilane:['general'], 'p-anisic acid':['preservative support'], 'polyquaternium-39':['conditioning polymer'], 'polyquaternium-53':['conditioning polymer'], 'ppg-5-ceteth-20':['general'], 'snail secretion filtrate':['animal extract'], 'synthetic beeswax':['texture agent'] });
const EXPECTED_WAVE24_BENCHMARK_CLOSURE = Object.freeze({ 'hexadecyloxy pg hydroxyethyl hexadecanamide':'moisturizer', 'peg-6 caprylic/capric glycerides':'emulsifier', 'sodium lauroyl lactylate':'emulsifier', 'zinc oxide':'uv filter', 'zea mays starch':'viscosity adjuster', 'peg-8':'humectant', 'microcrystalline wax':'viscosity adjuster', 'benzyl alcohol':'preservative' });
const EXPECTED_WAVE24_BENCHMARK_RAW = Object.freeze({ 'hexadecyloxy pg hydroxyethyl hexadecanamide':['barrier lipid'], 'peg-6 caprylic/capric glycerides':['surfactant','emulsifier'], 'sodium lauroyl lactylate':['surfactant','emulsifier'], 'zinc oxide':['uv filter','colorant'], 'zea mays starch':['powder'], 'peg-8':['humectant','solvent'], 'microcrystalline wax':['wax','texture agent'], 'benzyl alcohol':['preservative','fragrance'] });
const EXPECTED_ALL = Object.freeze({ ...EXPECTED_WAVE1, ...EXPECTED_WAVE2, ...EXPECTED_WAVE3, ...EXPECTED_WAVE4, ...EXPECTED_WAVE5, ...EXPECTED_WAVE6, ...EXPECTED_WAVE7, ...EXPECTED_WAVE8, ...EXPECTED_WAVE9, ...EXPECTED_WAVE10, ...EXPECTED_WAVE11, ...EXPECTED_WAVE12, ...EXPECTED_WAVE13, ...EXPECTED_WAVE14, ...EXPECTED_WAVE15, ...EXPECTED_WAVE16, ...EXPECTED_WAVE17, ...EXPECTED_WAVE18, ...EXPECTED_WAVE19, ...EXPECTED_WAVE20, ...EXPECTED_WAVE21, ...EXPECTED_WAVE22, ...EXPECTED_WAVE23, ...EXPECTED_WAVE24, ...EXPECTED_WAVE24_BENCHMARK_CLOSURE });
...
assert.deepEqual(Object.fromEntries(Object.entries(evidence).map(([key,item])=>[key,item.category])),EXPECTED_ALL,'verified category evidence must remain the reviewed cumulative wave 1 through wave 24 set');
...
let wave24BenchmarkClosureReviewed=0;
...
for(const [canonical,expectedCategory] of Object.entries(EXPECTED_ALL)){
  ...
  if(rawCategories.length>0 && !Object.hasOwn(EXPECTED_WAVE15,canonical) && !Object.hasOwn(EXPECTED_WAVE16,canonical) && !Object.hasOwn(EXPECTED_WAVE18,canonical) && !Object.hasOwn(EXPECTED_WAVE19,canonical) && !Object.hasOwn(EXPECTED_WAVE20,canonical) && !Object.hasOwn(EXPECTED_WAVE21,canonical) && !Object.hasOwn(EXPECTED_WAVE22,canonical) && !Object.hasOwn(EXPECTED_WAVE23,canonical) && !Object.hasOwn(EXPECTED_WAVE24,canonical) && !Object.hasOwn(EXPECTED_WAVE24_BENCHMARK_CLOSURE,canonical)) assert.ok(rawCategories.includes(expectedCategory.toLowerCase()),`${canonical}: verified category conflicts with existing raw category metadata (${rawCategories.join(', ')})`);
  ...
  if(Object.hasOwn(EXPECTED_WAVE24,canonical)){
    assert.equal(hasRawMissing,false,`${canonical}: final wave must preserve the existing unsupported raw category without inventing a missing row`);
    assert.deepEqual(rawCategories,EXPECTED_WAVE24_LEGACY_CATEGORIES[canonical],`${canonical}: final wave must preserve the frozen unsupported legacy category hint for audit`);
    wave24Reviewed+=1;
  }
  if(Object.hasOwn(EXPECTED_WAVE24_BENCHMARK_CLOSURE,canonical)){
    assert.deepEqual(rawCategories,EXPECTED_WAVE24_BENCHMARK_RAW[canonical],`${canonical}: benchmark closure must preserve the frozen raw category inventory`);
    if(canonical === 'benzyl alcohol') assert.equal(hasRawMissing,true,`${canonical}: duplicate group must retain its category-empty raw row`);
    else assert.equal(hasRawMissing,false,`${canonical}: benchmark closure must not invent a missing raw category row`);
    wave24BenchmarkClosureReviewed+=1;
  }
}
...
assert.equal(Object.keys(EXPECTED_WAVE24).length,7);
assert.equal(Object.keys(EXPECTED_WAVE24_BENCHMARK_CLOSURE).length,8);
...
assert.equal(wave24Reviewed,7,'wave 24 must review the original seven official-label public-role gaps');
assert.equal(wave24BenchmarkClosureReviewed,8,'wave 24 benchmark closure must resolve all eight additional recognized role gaps');
