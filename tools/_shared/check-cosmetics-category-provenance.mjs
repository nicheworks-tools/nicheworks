...
const EXPECTED_WAVE21 = Object.freeze({ triethoxycaprylylsilane:'binder' });
const EXPECTED_WAVE21_LEGACY_CATEGORIES = Object.freeze({ triethoxycaprylylsilane:['general'] });
const EXPECTED_ALL = Object.freeze({ ...EXPECTED_WAVE1, ...EXPECTED_WAVE2, ...EXPECTED_WAVE3, ...EXPECTED_WAVE4, ...EXPECTED_WAVE5, ...EXPECTED_WAVE6, ...EXPECTED_WAVE7, ...EXPECTED_WAVE8, ...EXPECTED_WAVE9, ...EXPECTED_WAVE10, ...EXPECTED_WAVE11, ...EXPECTED_WAVE12, ...EXPECTED_WAVE13, ...EXPECTED_WAVE14, ...EXPECTED_WAVE15, ...EXPECTED_WAVE16, ...EXPECTED_WAVE17, ...EXPECTED_WAVE18, ...EXPECTED_WAVE19, ...EXPECTED_WAVE20, ...EXPECTED_WAVE21 });
...
assert.deepEqual(Object.fromEntries(Object.entries(evidence).map(([key,item])=>[key,item.category])),EXPECTED_ALL,'verified category evidence must remain the reviewed cumulative wave 1 through wave 21 set');
...
for(const [canonical,expectedCategory] of Object.entries(EXPECTED_ALL)){
  ...
  if(Object.hasOwn(EXPECTED_WAVE21,canonical)){
    ...
    assert.deepEqual(rawCategories,EXPECTED_WAVE21_LEGACY_CATEGORIES[canonical],`${canonical}: wave 21 must preserve the frozen general category hint for audit`);
    wave21Reviewed+=1;
  }
}
...
assert.equal(Object.keys(EXPECTED_WAVE21).length,1);
...
assert.equal(wave21Reviewed,1,'wave 21 must review exactly one high-frequency binder public-role gap');
