...
const PRIOR_VERIFIED_NOTE_COUNT = 34;
const EXPECTED_WAVE9 = Object.freeze({
  chlorphenesin: Object.freeze({
    note_short: 'Preservative and antimicrobial ingredient; COSMILE Europe lists Chlorphenesin as protecting cosmetics from microbial spoilage and helping control microorganism growth.',
    source: 'https://cosmileeurope.eu/inci/detail/3114/chlorphenesin/',
    authority: 'Cosmetics Europe / COSMILE Europe'
  }),
  'cetearyl alcohol': Object.freeze({
    note_short: 'Fatty-alcohol mixture used for emollience, emulsion stability and viscosity control; COSMILE Europe also lists cleansing, emulsifying and foam-boosting surfactant functions.',
    source: 'https://cosmileeurope.eu/inci/detail/2895/cetearyl-alcohol/',
    authority: 'Cosmetics Europe / COSMILE Europe'
  }),
  'disodium edta': Object.freeze({
    note_short: 'Chelating ingredient; COSMILE Europe lists Disodium EDTA as binding metal ions that can affect cosmetic stability or appearance, with an additional viscosity-controlling function.',
    source: 'https://cosmileeurope.eu/inci/detail/4934/disodium-edta/',
    authority: 'Cosmetics Europe / COSMILE Europe'
  }),
  panthenol: Object.freeze({
    note_short: 'Provitamin B5 skin- and hair-conditioning ingredient; COSMILE Europe also describes humectant and skin-smoothing properties for Panthenol.',
    source: 'https://cosmileeurope.eu/inci/detail/10243/panthenol/',
    authority: 'Cosmetics Europe / COSMILE Europe'
  })
});
...
assert.ok(
  Object.keys(evidence).length >= PRIOR_VERIFIED_NOTE_COUNT + Object.keys(EXPECTED_WAVE9).length,
  'wave 9 reviewed identities must remain present as later verified-note waves extend the overlay'
);

for (const [canonical, expected] of Object.entries(EXPECTED_WAVE9)) {
  const item = evidence[canonical];
  assert.ok(item, `${canonical}: wave 9 verified note evidence missing`);
...
for (const [canonical, expected] of Object.entries(EXPECTED_WAVE9)) {
  ...
  assert.equal(item.note_short, expected.note_short, `${canonical}: runtime note must equal the reviewed wave 9 note`);
  ...
  assert.equal(item.category_verified, true, `${canonical}: wave 9 note identity must already have a verified public role`);
  assert.ok(Array.isArray(item.jp) && item.jp.some((value) => normalizeText(value)), `${canonical}: wave 9 note identity must retain a Japanese name`);
}

console.log(JSON.stringify({
  status: 'pass',
  phase: 'verified-note-wave-9-common-label-priority',
  prior_verified_note_identities: PRIOR_VERIFIED_NOTE_COUNT,
  wave_9_verified_note_identities: Object.keys(EXPECTED_WAVE9).length,
  cumulative_verified_note_identities: Object.keys(evidence).length,
  wave_9_identities: Object.keys(EXPECTED_WAVE9),
