...
const PRIOR_VERIFIED_NOTE_COUNT = 46;
const EXPECTED_WAVE12 = Object.freeze({
  cholesterol: Object.freeze({
    note_short: 'Skin-conditioning emollient; COSMILE Europe also lists Cholesterol for emulsifying, viscosity control and light stabilisation.',
    source: 'https://cosmileeurope.eu/inci/detail/3117/cholesterol/',
    authority: 'Cosmetics Europe / COSMILE Europe'
  }),
  arginine: Object.freeze({
    note_short: 'Amino acid used for skin and hair conditioning; COSMILE Europe also lists an antistatic function for Arginine.',
    source: 'https://cosmileeurope.eu/inci/detail/1160/arginine/',
    authority: 'Cosmetics Europe / COSMILE Europe'
  }),
  'benzoic acid': Object.freeze({
    note_short: 'Preservative and pH-adjusting ingredient; COSMILE Europe also lists a fragrance function for Benzoic Acid.',
    source: 'https://cosmileeurope.eu/inci/detail/1569/benzoic-acid/',
    authority: 'Cosmetics Europe / COSMILE Europe'
  }),
  'butylene glycol': Object.freeze({
    note_short: 'Humectant, skin-conditioning solvent and viscosity-controlling ingredient; COSMILE Europe lists these functions for Butylene Glycol.',
    source: 'https://cosmileeurope.eu/inci/detail/2015/butylene-glycol/',
    authority: 'Cosmetics Europe / COSMILE Europe'
  })
});
...
assert.ok(
  Object.keys(evidence).length >= PRIOR_VERIFIED_NOTE_COUNT + Object.keys(EXPECTED_WAVE12).length,
  'wave 12 reviewed identities must remain present as later verified-note waves extend the overlay'
);

for (const [canonical, expected] of Object.entries(EXPECTED_WAVE12)) {
  const item = evidence[canonical];
  assert.ok(item, `${canonical}: wave 12 verified note evidence missing`);
...
for (const [canonical, expected] of Object.entries(EXPECTED_WAVE12)) {
  ...
  assert.equal(item.note_short, expected.note_short, `${canonical}: runtime note must equal the reviewed wave 12 note`);
  ...
  assert.ok(normalizeText(item.category), `${canonical}: wave 12 note identity must retain a supported public role`);
  assert.ok(Array.isArray(item.jp) && item.jp.some((value) => normalizeText(value)), `${canonical}: wave 12 note identity must retain a Japanese name`);
}

console.log(JSON.stringify({
  status: 'pass',
  phase: 'verified-note-wave-12-common-label-priority',
  prior_verified_note_identities: PRIOR_VERIFIED_NOTE_COUNT,
  wave_12_verified_note_identities: Object.keys(EXPECTED_WAVE12).length,
  cumulative_verified_note_identities: Object.keys(evidence).length,
  wave_12_identities: Object.keys(EXPECTED_WAVE12),
