...
const EXPECTED_CATEGORIES = new Set([
  ...
  'skin conditioning', 'emollient', 'cleanser', 'emulsifier', 'smoothing', 'binder', 'film former', 'hair conditioning', 'fragrance', 'moisturizer', 'uv filter'
]);
...
const EXPECTED_WAVE24_PROMOTED = new Set(['triethoxycaprylylsilane', 'p-anisic acid', 'polyquaternium-39', 'polyquaternium-53', 'ppg-5-ceteth-20', 'snail secretion filtrate', 'synthetic beeswax']);
const EXPECTED_WAVE24_BENCHMARK_CLOSURE = new Set(['hexadecyloxy pg hydroxyethyl hexadecanamide', 'peg-6 caprylic/capric glycerides', 'sodium lauroyl lactylate', 'zinc oxide', 'zea mays starch', 'peg-8', 'microcrystalline wax', 'benzyl alcohol']);
...
assert.equal(authorityFunctionToCategory.get('uv filter'), 'uv filter', 'COSMILE UV FILTER must map explicitly to uv filter');
assert.equal(authorityFunctionToCategory.size, 26, 'authority function vocabulary must contain exactly 26 reviewed terms');
...
for (const canonical of EXPECTED_WAVE24_BENCHMARK_CLOSURE) assert.ok(runtimeMappings[canonical], `${canonical}: wave 24 benchmark-closure mapping must be runtime_verified`);
...
assert.equal(runtimeMappings['silica']?.source_functions?.[0], 'viscosity controlling', 'Silica must use the reviewed COSMILE VISCOSITY CONTROLLING authority term');
assert.equal(runtimeMappings['alumina']?.source_functions?.[0], 'viscosity controlling', 'Alumina must use the reviewed COSMILE VISCOSITY CONTROLLING authority term');
assert.equal(runtimeMappings['aluminum stearate']?.source_functions?.[0], 'viscosity controlling', 'Aluminum Stearate must use the reviewed COSMILE VISCOSITY CONTROLLING authority term');
assert.equal(runtimeMappings['dimethicone crosspolymer']?.source_functions?.[0], 'viscosity controlling', 'Dimethicone Crosspolymer must use the reviewed COSMILE VISCOSITY CONTROLLING authority term');
...
assert.equal(runtimeMappings['helianthus annuus sunflower seed wax']?.source_functions?.[0], 'skin conditioning', 'Sunflower Seed Wax must use the reviewed COSMILE SKIN CONDITIONING authority term');
assert.equal(runtimeMappings['melaleuca alternifolia tea tree leaf oil']?.source_functions?.[0], 'antioxidant', 'Tea Tree Leaf Oil must use the reviewed COSMILE ANTIOXIDANT authority term');
assert.equal(runtimeMappings['peg-120 methyl glucose dioleate']?.source_functions?.[0], 'surfactant - emulsifying', 'PEG-120 Methyl Glucose Dioleate must use the reviewed COSMILE SURFACTANT - EMULSIFYING authority term');
assert.equal(runtimeMappings['peg-30 dipolyhydroxystearate']?.source_functions?.[0], 'surfactant - emulsifying', 'PEG-30 Dipolyhydroxystearate must use the reviewed COSMILE SURFACTANT - EMULSIFYING authority term');
assert.equal(runtimeMappings['pentaerythrityl tetraethylhexanoate']?.source_functions?.[0], 'skin conditioning - emollient', 'Pentaerythrityl Tetraethylhexanoate must use the reviewed COSMILE SKIN CONDITIONING - EMOLLIENT authority term');
assert.equal(runtimeMappings['polyacrylate crosspolymer-11']?.source_functions?.[0], 'viscosity controlling', 'Polyacrylate Crosspolymer-11 must use the reviewed COSMILE VISCOSITY CONTROLLING authority term');
assert.equal(runtimeMappings['polyglyceryl-4 caprate']?.source_functions?.[0], 'surfactant - emulsifying', 'Polyglyceryl-4 Caprate must use the reviewed COSMILE SURFACTANT - EMULSIFYING authority term');
assert.equal(runtimeMappings['sphingolipids']?.source_functions?.[0], 'skin conditioning', 'Sphingolipids must use the reviewed COSMILE SKIN CONDITIONING authority term');
assert.equal(runtimeMappings['triethoxycaprylylsilane']?.source_functions?.[0], 'binding', 'Triethoxycaprylylsilane must use the reviewed COSMILE BINDING authority term');
assert.equal(runtimeMappings['p-anisic acid']?.source_functions?.[0], 'fragrance', 'p-Anisic Acid must use the reviewed COSMILE FRAGRANCE authority term');
assert.equal(runtimeMappings['polyquaternium-39']?.source_functions?.[0], 'film forming', 'Polyquaternium-39 must use the reviewed COSMILE FILM FORMING authority term');
assert.equal(runtimeMappings['polyquaternium-53']?.source_functions?.[0], 'hair conditioning', 'Polyquaternium-53 must use the reviewed COSMILE HAIR CONDITIONING authority term');
assert.equal(runtimeMappings['ppg-5-ceteth-20']?.source_functions?.[0], 'surfactant - emulsifying', 'PPG-5-Ceteth-20 must use the reviewed COSMILE SURFACTANT - EMULSIFYING authority term');
assert.equal(runtimeMappings['snail secretion filtrate']?.source_functions?.[0], 'skin conditioning - miscellaneous', 'Snail Secretion Filtrate must use the reviewed KCIA skin-conditioning function');
assert.equal(runtimeMappings['synthetic beeswax']?.source_functions?.[0], 'viscosity controlling', 'Synthetic Beeswax must use the reviewed COSMILE VISCOSITY CONTROLLING authority term');
...
console.log(JSON.stringify({
  status: 'pass',
  phase: 'verified-category-taxonomy-wave-24-final-official-label-coverage',
  ...
  wave_24_benchmark_closure_mappings: [...EXPECTED_WAVE24_BENCHMARK_CLOSURE],
  official_label_role_gap_target: 0,
