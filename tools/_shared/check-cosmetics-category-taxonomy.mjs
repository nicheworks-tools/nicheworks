...
const EXPECTED_CATEGORIES = new Set([
  ...
  'skin conditioning', 'emollient', 'cleanser', 'emulsifier', 'smoothing', 'binder'
]);
...
const EXPECTED_WAVE21_PROMOTED = new Set(['triethoxycaprylylsilane']);
...
assert.equal(authorityFunctionToCategory.size, 21, 'authority function vocabulary must contain exactly 21 reviewed terms');
