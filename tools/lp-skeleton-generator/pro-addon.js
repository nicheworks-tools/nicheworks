(() => {
  ...
  function render(){box();document.body.classList.toggle('is-pro',isPro());document.querySelectorAll('[data-pro-only]').forEach(el=>el.style.display=isPro()?'':'none');document.querySelectorAll('[data-pro-lock]').forEach(el=>el.style.display=isPro()?'none':'');const m=document.getElementById('lpProMsg');if(m)m.textContent=isPro()?'Pro active: Markdown and HTML exports are unlocked. / Pro解放済み。':'Pro locks Markdown and HTML exports. / ProでMarkdown/HTML出力を解放。'}
