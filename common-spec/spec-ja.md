...
1. 英語版のファイル構成は日本語版と同一とし、
   **`index.html` → `/en/index.html`**
   **`usage.html` → `/en/usage.html`**
   にコピーして作成する。

2. HTML の `<html lang="ja">` を **`lang="en"` に変更**する以外、
   **レイアウト・CSS・テンプレ構造は全て日本語版と共通**とする。

3. 文言（タイトル・description・本文・UI表示文）は
   **人間読み可能な英語に置換するだけ**でよい。

4. 英語版では **日本語要素を一切残さない**（SEO・重複防止のため）。
...
6. 多言語切替UIが必要な場合は、
   **ヘッダー下 or フッター上部**に小型リンクで実装する（任意）。
